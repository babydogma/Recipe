/* Portionly — static app logic, Figma-style dashboard + recipe dropdown cards + cart planning v84 */
(() => {
  "use strict";
  ...
  const CATEGORY_TILE_MEDIA = {
    all: { image: "assets/category-tiles/all-dishes.webp", position: "center 46%" },
    favorites: { image: "assets/category-tiles/favorites.webp", position: "center 48%" },
    breakfast: { image: "assets/category-tiles/breakfast.webp", position: "center 42%" },
    main: { image: "assets/category-tiles/lunch.webp", position: "center 44%" },
    dinner: { image: "assets/category-tiles/dinner.webp", position: "center 40%" },
    dessert: { image: "assets/category-tiles/dessert.webp", position: "center 44%" }
  };
  ...
  function renderCategories() {
    ...
    const mealTile = (category, icon) => tile(category, { size: "meal", icon });

    state.refs.categoryTabs.innerHTML = `
      <div class="dashboard-menu">
        <button class="dashboard-hero-card" type="button" data-dashboard-action="ideas" style="--tile-image: url('assets/category-tiles/lunch.webp'); --tile-image-position: center 45%;">
          <span class="dashboard-hero-card__content">
            <span class="dashboard-kicker">Идея на сегодня</span>
            <strong>Что приготовить сегодня?</strong>
            <span>3 быстрые идеи из твоего каталога</span>
            <em>Посмотреть идеи ${dashboardArrowTemplate()}</em>
          </span>
          <span class="dashboard-hero-card__dots" aria-hidden="true"><i></i><i></i><i></i></span>
        </button>

        <section class="dashboard-section" aria-label="Быстрый доступ">
          <h2>Быстрый доступ</h2>
          <div class="dashboard-quick-grid">
            ${tile(byId("all"), { size: "quick", icon: "grid" })}
            ${tile(byId("favorites"), { size: "quick", icon: "heart" })}
          </div>
        </section>
        ...
        <section class="dashboard-section" aria-label="Приёмы пищи">
          ...
          <div class="dashboard-meal-grid">
            ${mealTile(byId("breakfast"), "sun")}
            ${mealTile(byId("main"), "sun")}
            ${mealTile(byId("dinner"), "moon")}
            ${mealTile(byId("dessert"), "dessert")}
          </div>
        </section>
        ...
        <section class="dashboard-section" aria-label="Планирование и покупки">
          ...
          <div class="dashboard-planning-grid">
            <button class="dashboard-shopping-card" type="button" data-dashboard-action="basket" style="--tile-image: url('assets/chicken-lavash-vegetables-yogurt.jpg'); --tile-image-position: center 52%;">
              <span class="dashboard-card__icon" aria-hidden="true">${dashboardIconTemplate("bag")}</span>
              <span class="dashboard-card__copy">
                <strong>Список покупок</strong>
                <span>${productCount} ${plural(productCount, "продукт", "продукта", "продуктов")}</span>
                <small>Из выбранных блюд</small>
              </span>
            </button>
            <div class="dashboard-filter-stack">
              <button class="dashboard-mini-action" type="button" data-smart-filter="quick">
                <span class="dashboard-card__icon" aria-hidden="true">${dashboardIconTemplate("clock")}</span>
                <span><strong>Быстрые блюда</strong><small>до 20 минут</small></span>
                ${dashboardArrowTemplate()}
              </button>
              <button class="dashboard-mini-action" type="button" data-smart-filter="lowcal">
                <span class="dashboard-card__icon dashboard-card__icon--green" aria-hidden="true">${dashboardIconTemplate("leaf")}</span>
                <span><strong>Низкокалорийные</strong><small>до 350 ккал</small></span>
                ${dashboardArrowTemplate()}
              </button>
            </div>
          </div>
  ...
  function dashboardArrowTemplate() {
    return `
      <svg class="dashboard-arrow" viewBox="0 0 24 24" focusable="false" aria-hidden="true">
        <path d="M5 12h13" />
  ...
  function dashboardIconTemplate(icon) {
    ...
    const icons = {
      grid: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M5 5h5v5H5z"/><path d="M14 5h5v5h-5z"/><path d="M5 14h5v5H5z"/><path d="M14 14h5v5h-5z"/></svg>`,
      heart: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M20.3 8.7c0 5.1-8.3 10-8.3 10s-8.3-4.9-8.3-10A4.5 4.5 0 0 1 12 6.1a4.5 4.5 0 0 1 8.3 2.6Z"/></svg>`,
      sun: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M12 16a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z"/><path d="M12 2.8v2.1"/><path d="M12 19.1v2.1"/><path d="M4.9 4.9l1.5 1.5"/><path d="M17.6 17.6l1.5 1.5"/><path d="M2.8 12h2.1"/><path d="M19.1 12h2.1"/><path d="M4.9 19.1l1.5-1.5"/><path d="M17.6 6.4l1.5-1.5"/></svg>`,
      moon: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M19 15.6A7.4 7.4 0 0 1 8.4 5a7.8 7.8 0 1 0 10.6 10.6Z"/></svg>`,
      dessert: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M7 9h10l-1 10H8L7 9Z"/><path d="M9 9V7.4a3 3 0 0 1 6 0V9"/><path d="M6 9h12"/></svg>`,
      bag: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M5.5 8.5h13l-1 11h-11l-1-11Z"/><path d="M9 8.5a3 3 0 0 1 6 0"/></svg>`,
      clock: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Z"/><path d="M12 7.5V12l3 2"/></svg>`,
      leaf: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M20 4C11.5 4.2 6.2 8.1 5.2 16.5c8.3 1.1 12.6-3.8 14.8-12.5Z"/><path d="M5 19c2.8-4.7 6.2-7.8 10.3-9.4"/></svg>`
    };
