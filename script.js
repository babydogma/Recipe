...
(() => {
  ...
  const STORAGE_COOKED_HISTORY = "portionly:cooked-history:v1";
  ...
  function init() {
    ...
    state.cookedHistory = normalizeCookedHistory(loadJson(STORAGE_COOKED_HISTORY, []));
  ...
  function cacheDom() {
    state.refs = {
      ...
      categoryFilterMain: document.getElementById("categoryFilterMain"),
  ...
  function bindStaticEvents() {
    ...
    r.categoryFilterTabs?.addEventListener("scroll", updateCategoryEdgeMasks, { passive: true });
    r.categoryFilterTabs?.addEventListener("click", handleCategoryFilterClick);
  ...
  function renderTopbarState() {
    ...
    updateTopbarTitle(listMode);
  }

  function updateTopbarTitle(listMode) {
    const titleEl = state.refs.topbar?.querySelector(".brand h1");
    const subtitleEl = state.refs.topbar?.querySelector(".brand p");
    if (!titleEl || !subtitleEl) return;

    if (!listMode) {
      titleEl.textContent = "Portionly";
      subtitleEl.textContent = "Планировщик блюд и закупки продуктов";
      return;
    }

    titleEl.textContent = getCurrentSectionTitle();
  ...
  function getCurrentSectionTitle() {
    ...
    if (state.smartFilter === "quick") return "Быстрые блюда";
    if (state.smartFilter === "lowcal") return "Низкокалорийные";
    if (state.activeCategory === "favorites") return "Избранное";
  ...
  function renderSearchState() {
    ...
    const categoryFilterMain = state.refs.categoryFilterMain;
    if (!shell || !panel) return;

    const hasQuery = Boolean(state.search.trim());
    const categoryConfig = !state.dashboardOpen && !state.smartFilter && !state.selectedOnly
      ? getCategoryOverviewConfig(state.activeCategory)
      : null;
    const categoryFilterMode = Boolean(categoryConfig);
    const categorySearchActive = categoryFilterMode && (state.searchOpen || hasQuery);

    renderCategoryFilterTabs(categoryConfig);

    shell.classList.toggle("search-open", state.dashboardOpen || state.searchOpen || hasQuery);
    ...
    shell.classList.toggle("category-filter-mode", categoryFilterMode);
    shell.classList.toggle("category-search-active", categorySearchActive);

    panel.hidden = !(state.dashboardOpen || state.searchOpen || hasQuery || categoryFilterMode);
    panel.setAttribute("aria-hidden", categoryFilterMode && !categorySearchActive ? "true" : "false");
    if (state.refs.searchInput) state.refs.searchInput.tabIndex = categoryFilterMode && !categorySearchActive ? -1 : 0;

    if (categoryFilterMain) categoryFilterMain.hidden = !categoryFilterMode;
    if (clearBtn) clearBtn.hidden = state.dashboardOpen ? !hasQuery : !(state.searchOpen || hasQuery);
  ...
  function renderCategoryFilterTabs(config) {
    const root = state.refs.categoryFilterTabs;
    if (!root) return;
    ...
    const activeFilter = getActiveCategoryOverviewFilter(filters);
  ...
  function getActiveCategoryOverviewFilter(filters) {
    const safeFilters = Array.isArray(filters) && filters.length ? filters : [{ id: "all", label: "Все" }];
    const activeFilter = safeFilters.some(item => item.id === state.categoryPageFilter) ? state.categoryPageFilter : "all";
  ...
  function updateCategoryEdgeMasks() {
    updateScrollableFilterMask(state.refs.categoryTabs);
    updateScrollableFilterMask(state.refs.categoryFilterTabs);
  }

  function updateScrollableFilterMask(tabs) {
    if (!tabs) return;
  ...
  function categoryOverviewTemplate(config, totalCount, featured, otherRecipes) {
    getActiveCategoryOverviewFilter(config.filters || [{ id: "all", label: "Все" }]);
    ...
    return `
      <section class="category-feature-section" aria-label="${escapeHTML(config.pickTitle)}">
        <div class="category-feature-label">
          ...
          ${config.pickSubtitle ? `<span>${escapeHTML(config.pickSubtitle)}</span>` : ""}
        </div>

        <article class="category-feature-card ${state.selectedIds.includes(recipe.id) ? "selected" : ""}" data-card-id="${escapeHTML(recipe.id)}" data-page-recipe-id="${escapeHTML(recipe.id)}" data-action="open-detail" data-recipe-id="${escapeHTML(recipe.id)}" role="button" tabindex="0">
  ...
  function profileScreenShell(content, label = "Профиль") {
    return `
      <article class="recipe-screen__panel profile-screen" role="dialog" aria-modal="true" aria-label="${escapeHTML(label)}">
        ${content}
      </article>`;
  }
  ...
  function openProfileScreen() {
    ...
    state.activeRecipeId = "__profile__";
    screen.innerHTML = profileScreenTemplate();
    screen.hidden = false;
  ...
  function profileScreenTemplate() {
    ...
    const monthEntries = getCurrentMonthCookedHistory(history);
    const statsSource = monthEntries.length ? monthEntries : history;
    const totalMonth = monthEntries.length;
    const avg = getAverageRating(statsSource);
    const rated = statsSource.filter(item => Number(item.rating) > 0);
    const recentEntries = history.slice(0, 5);
    const recommendations = getProfileRecommendations(3);
    const heroRecipe = getLatestProfileHeroRecipe(history) || recommendations[0] || compactRecipeVariants(getRecipes())[0] || null;
    const heroImage = heroRecipe ? getRecipeImage(heroRecipe) : "";
    const topCategory = getTopCookedCategory(statsSource);
    const regularity = getCurrentMonthRegularity(monthEntries);
    const heroStyle = `${heroImage ? `--profile-hero-image: url('${escapeHTML(heroImage)}');` : ""} --profile-regularity: ${regularity.percent}%;`;
    ...
    return profileScreenShell(`
      <header class="profile-dashboard-head">
        ...
        <button class="recipe-screen__round profile-settings" type="button" data-action="profile-settings" aria-label="Настройки профиля">
          <svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
            <path d="M12 15.2a3.2 3.2 0 1 0 0-6.4 3.2 3.2 0 0 0 0 6.4Z"/>
            <path d="M19.4 13.5a7.8 7.8 0 0 0 .05-2.9l2-1.55-2-3.45-2.45 1a8.1 8.1 0 0 0-2.5-1.45L14.15 2h-4.3L9.5 5.15A8.1 8.1 0 0 0 7 6.6l-2.45-1-2 3.45 2 1.55a7.8 7.8 0 0 0 .05 2.9l-2.05 1.6 2 3.45 2.5-1.02a8 8 0 0 0 2.45 1.4L9.85 22h4.3l.35-3.07a8 8 0 0 0 2.45-1.4l2.5 1.02 2-3.45-2.05-1.6Z"/>
          </svg>
        </button>
      </header>

      <section class="profile-dashboard-hero" style="${heroStyle}" aria-label="Сводка профиля">
        <div class="profile-dashboard-hero__shade"></div>
        <div class="profile-dashboard-hero__stats">
          <span>Этот месяц</span>
          <strong>${totalMonth}</strong>
          <small>${plural(totalMonth, "приготовление", "приготовления", "приготовлений")}</small>
          <hr>
          <small>средняя оценка</small>
          <strong>${rated.length ? formatRating(avg) : "—"}</strong>
          <hr>
          <small>любимая категория</small>
          <button type="button" data-action="profile-category" data-category-id="${escapeHTML(topCategory.id)}">
            ${escapeHTML(topCategory.title)} <b aria-hidden="true">♡</b>
          </button>
        </div>
        <div class="profile-dashboard-hero__ring" aria-label="Регулярность ${regularity.percent}%">
          <strong>${regularity.percent}<span>%</span></strong>
          <small>регулярность</small>
        </div>
      </section>

      ${profileWeekStripTemplate(history)}

      <section class="profile-section profile-section--history">
        <div class="profile-section__head profile-section__head--action">
          <h3>История</h3>
          <button type="button" data-action="profile-category" data-category-id="all">Смотреть всё <span aria-hidden="true">›</span></button>
        </div>
        <div class="profile-timeline-list">
          ${recentEntries.length
            ? recentEntries.map(entry => profileTimelineEntryTemplate(entry)).join("")
      ...
      <section class="profile-section">
        <div class="profile-section__head">
          <h3>Паттерны месяца</h3>
        </div>
        ${profilePatternCardsTemplate(monthEntries, history)}
      </section>
  ...
  function getCurrentMonthCookedHistory(history = getCookedHistory()) {
    const now = new Date();
  ...
  function getLatestProfileHeroRecipe(history = getCookedHistory()) {
    const latestEntry = history.find(entry => getRecipe(entry.recipeId));
    return latestEntry ? getRecipe(latestEntry.recipeId) : null;
  }

  function getCookedDayKey(value) {
  ...
  function getCurrentMonthRegularity(monthEntries = getCurrentMonthCookedHistory()) {
    const now = new Date();
    const elapsedDays = Math.max(1, now.getDate());
    const uniqueDays = new Set(monthEntries.map(entry => getCookedDayKey(entry.cookedAt)).filter(Boolean));
    const percent = Math.max(0, Math.min(100, Math.round((uniqueDays.size / elapsedDays) * 100)));
    return { percent, uniqueDays: uniqueDays.size };
  }

  function getTopCookedCategory(entries = getCookedHistory()) {
    ...
    const category = getCategories().find(item => item.id === topId) || getCategories().find(item => item.id !== "favorites") || { id: "all", title: "Все блюда" };
    return { id: category.id, title: category.title, count: topCount };
  }

  function profileWeekStripTemplate(history = getCookedHistory()) {
    ...
    const cookedDays = new Set(history.map(entry => getCookedDayKey(entry.cookedAt)).filter(Boolean));

    const days = names.map((name, index) => {
      const date = new Date(monday);
      date.setDate(monday.getDate() + index);
      const key = getCookedDayKey(date);
      const isToday = key === getCookedDayKey(now);
      const isActive = cookedDays.has(key);
      return `
        <span class="profile-week-day ${isActive ? "is-active" : ""} ${isToday ? "is-today" : ""}">
          <b>${name}</b>
    ...
    return `
      <section class="profile-week-card" aria-label="Активность за неделю">
        <div class="profile-week-card__days">${days}</div>
        <div class="profile-week-card__chart" aria-hidden="true"><i></i><i></i><i></i></div>
      </section>`;
  }

  function profileTimelineEntryTemplate(entry) {
    const recipe = getRecipe(entry.recipeId);
    if (!recipe) return "";
    const image = getRecipeImage(recipe);
    const rating = Number(entry.rating || 0);
    const meta = `${formatNumber(recipe.nutrition?.kcal || 0)} ккал · ${rating ? `${formatRating(rating)} ★` : "без оценки"}`;

    return `
      <button class="profile-timeline-row" type="button" data-action="profile-recipe" data-recipe-id="${escapeHTML(recipe.id)}">
        <span class="profile-timeline-row__date">
          <b>${escapeHTML(formatCookedDayMonth(entry.cookedAt))}</b>
          <small>${escapeHTML(formatCookedTime(entry.cookedAt))}</small>
        </span>
        ...
        <b class="profile-timeline-row__arrow" aria-hidden="true">›</b>
      </button>`;
  }

  function profilePatternCardsTemplate(monthEntries = [], history = getCookedHistory()) {
    const source = monthEntries.length ? monthEntries : history;
    const topCategory = getTopCookedCategory(source);
    const bestRecipe = getBestRatedCookedRecipe(source) || getLatestProfileHeroRecipe(history) || getProfileRecommendations(1)[0];
    const bestStats = bestRecipe ? getCookedRecipeStats(bestRecipe.id) : null;
    const bestLabel = bestStats?.avg ? `Твой лучший рейтинг: ${formatRating(bestStats.avg)}` : "Можно оценить после готовки";

    return `
      <div class="profile-pattern-grid">
        <button class="profile-pattern-card" type="button" data-action="profile-category" data-category-id="${escapeHTML(topCategory.id)}">
          <span class="profile-pattern-card__icon" aria-hidden="true">
            <svg viewBox="0 0 24 24"><path d="M12 4v2.2M12 17.8V20M4 12h2.2M17.8 12H20M6.35 6.35l1.55 1.55M16.1 16.1l1.55 1.55M17.65 6.35 16.1 7.9M7.9 16.1l-1.55 1.55"/><path d="M12 16a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z"/></svg>
          </span>
          <span>
            <small>Чаще всего —</small>
            <strong>${escapeHTML(topCategory.title)}</strong>
            <em>${topCategory.count} ${plural(topCategory.count, "приготовление", "приготовления", "приготовлений")}</em>
          </span>
        </button>
        <button class="profile-pattern-card" type="button" data-action="${bestRecipe ? "profile-recipe" : "profile-home"}" ${bestRecipe ? `data-recipe-id="${escapeHTML(bestRecipe.id)}"` : ""}>
          <span class="profile-pattern-card__icon" aria-hidden="true">
            <svg viewBox="0 0 24 24"><path d="m12 3 2.82 5.72 6.31.92-4.56 4.44 1.08 6.28L12 17.39l-5.65 2.97 1.08-6.28-4.56-4.44 6.31-.92L12 3Z"/></svg>
          </span>
          <span>
            <small>Стоит повторить</small>
            <strong>${bestRecipe ? escapeHTML(bestRecipe.title) : "Пока пусто"}</strong>
            <em>${escapeHTML(bestLabel)}</em>
          </span>
        </button>
      </div>`;
  }

  function getBestRatedCookedRecipe(entries = getCookedHistory()) {
    const best = entries
      .filter(entry => Number(entry.rating) > 0 && getRecipe(entry.recipeId))
      .sort((a, b) => Number(b.rating) - Number(a.rating) || new Date(b.cookedAt) - new Date(a.cookedAt))[0];
    return best ? getRecipe(best.recipeId) : null;
  }

  function formatCookedDayMonth(value) {
    const date = new Date(value);
    if (Number.isNaN(date.getTime())) return "—";
    return date.toLocaleDateString("ru-RU", { day: "numeric", month: "short" }).replace(".", "");
  }

  function formatCookedTime(value) {
    const date = new Date(value);
    if (Number.isNaN(date.getTime())) return "—";
  ...
  function profileCategoryTilesTemplate() {
    const categories = getCategories().filter(category => category.id !== "favorites");
    return categories.map(category => {
      ...
      return `
        <button class="profile-category-tile ${media ? "profile-category-tile--image" : ""}" type="button" data-action="profile-category" data-category-id="${escapeHTML(category.id)}"${style}>
          <strong>${escapeHTML(category.title)}</strong>
          <span>${count} ${plural(count, "готовка", "готовки", "готовок")}</span>
        </button>`;
    }).join("");
  }

  function openProfileCategory(categoryId = "all") {
    const screen = state.refs.recipeScreen;
    if (!screen) return;

    state.activeRecipeId = "__profile__";
    screen.innerHTML = profileCategoryTemplate(categoryId);
    screen.hidden = false;
    screen.setAttribute("aria-hidden", "false");
    document.body.classList.add("recipe-screen-open");
  ...
  function profileCategoryTemplate(categoryId) {
    const category = getCategories().find(item => item.id === categoryId) || { id: "all", title: "Все блюда" };
    const recipes = getCookedRecipesByCategory(category.id);
    const count = getCookedCountByCategory(category.id);

    return profileScreenShell(`
      <header class="profile-head profile-head--compact">
        <button class="recipe-screen__round profile-back" type="button" data-action="profile-home" aria-label="Назад в профиль">
          <svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M15.2 5.4 8.6 12l6.6 6.6"/></svg>
        </button>
        <div>
          <span>Приготовлено</span>
          <h2>${escapeHTML(category.title)}</h2>
          <p>${count} ${plural(count, "приготовление", "приготовления", "приготовлений")}</p>
        </div>
      </header>
      ...
      <section class="profile-section">
        <div class="profile-dish-grid">
          ${recipes.length
            ? recipes.map(recipe => profileCookedCardTemplate(recipe)).join("")
            : `<div class="profile-empty profile-empty--wide">В этом разделе пока ничего не приготовлено.</div>`}
        </div>
      </section>
    `, category.title);
  }

  function profileCookedCardTemplate(recipe) {
    const image = getRecipeImage(recipe);
    const stats = getCookedRecipeStats(recipe.id);
    return `
      <button class="profile-dish-card" type="button" data-action="profile-recipe" data-recipe-id="${escapeHTML(recipe.id)}">
        <span class="profile-dish-card__image">
          ${image ? `<img src="${escapeHTML(image)}" alt="${escapeHTML(recipe.title)}" loading="lazy" decoding="async" onerror="window.handleRecipeImageError(this)">` : ""}
        </span>
        <span class="profile-dish-card__body">
          <strong>${escapeHTML(recipe.title)}</strong>
          <small>${stats.count} ${plural(stats.count, "раз", "раза", "раз")} · ${stats.avg ? `${formatRating(stats.avg)} ★` : "без оценки"}</small>
        </span>
      </button>`;
  }

  function openProfileRecipeStats(recipeId) {
    const recipe = getRecipe(recipeId);
    const screen = state.refs.recipeScreen;
    if (!recipe || !screen) return;

    state.activeRecipeId = "__profile__";
    screen.innerHTML = profileRecipeStatsTemplate(recipe);
    screen.hidden = false;
    screen.setAttribute("aria-hidden", "false");
    document.body.classList.add("recipe-screen-open");
  ...
  function profileRecipeStatsTemplate(recipe) {
    const image = getRecipeImage(recipe);
    const stats = getCookedRecipeStats(recipe.id);
    const entries = getCookedHistory(recipe.id).slice(0, 8);
    ...
    return profileScreenShell(`
      <header class="profile-head profile-head--compact">
        ...
        <div>
          ...
          <p>${stats.count ? `Последний раз: ${formatCookedDate(stats.lastAt)}` : "Ещё не готовилось"}</p>
        </div>
      </header>

      <section class="profile-recipe-hero">
        ${image ? `<img src="${escapeHTML(image)}" alt="${escapeHTML(recipe.title)}" decoding="async" onerror="window.handleRecipeImageError(this)">` : ""}
      </section>

      <section class="profile-stat-grid">
        <div><strong>${stats.count}</strong><span>${plural(stats.count, "готовка", "готовки", "готовок")}</span></div>
        <div><strong>${stats.avg ? formatRating(stats.avg) : "—"}</strong><span>средняя оценка</span></div>
      </section>

      ${profileRatingControlTemplate(recipe, entries[0] || null)}
          ...
          ${entries.length
            ? entries.map(entry => `
              <div class="profile-history-row">
                <span>${formatCookedDate(entry.cookedAt)}</span>
                <strong>${entry.rating ? `${formatRating(entry.rating)} ★` : "без оценки"}</strong>
              </div>`).join("")
            : `<div class="profile-empty">Записей пока нет.</div>`}
        </div>
      </section>

      <button class="profile-open-recipe-btn" type="button" data-action="profile-open-recipe" data-recipe-id="${escapeHTML(recipe.id)}">
        Открыть рецепт
      </button>
    `, recipe.title);
  }

  function profileRecipeRowTemplate(recipe, actionName) {
    const image = getRecipeImage(recipe);
    const stats = getCookedRecipeStats(recipe.id);
    const meta = stats.count
      ? `${stats.count} ${plural(stats.count, "готовка", "готовки", "готовок")} · ${stats.avg ? `${formatRating(stats.avg)} ★` : "без оценки"}`
      : categoryRecipeNutritionLine(recipe);

    return `
      <button class="profile-recipe-row" type="button" data-action="${escapeHTML(actionName)}" data-recipe-id="${escapeHTML(recipe.id)}">
        <span class="profile-recipe-row__image">
          ${image ? `<img src="${escapeHTML(image)}" alt="${escapeHTML(recipe.title)}" loading="lazy" decoding="async" onerror="window.handleRecipeImageError(this)">` : ""}
        </span>
        <span class="profile-recipe-row__body">
          <strong>${escapeHTML(recipe.title)}</strong>
          <small>${escapeHTML(meta)}</small>
        </span>
        <b aria-hidden="true">›</b>
  ...
  function markRecipeServed(recipeId) {
    const recipe = getRecipe(recipeId);
    if (!recipe) return;
    ...
    const entry = {
      ...
      portions: getTargetPortions(recipeId),
      rating: null
    };
  ...
  function openCookedRatingScreen(recipeId, entryId) {
    ...
    screen.innerHTML = cookedRatingTemplate(recipe, entryId);
    screen.hidden = false;
    screen.setAttribute("aria-hidden", "false");
    document.body.classList.add("recipe-screen-open");
  ...
  function profileRatingControlTemplate(recipe, latestEntry) {
    if (!latestEntry) {
      return `
        <section class="profile-rating-card profile-rating-card--empty">
          <span>Оценка блюда</span>
          <p>Сначала отметь блюдо как приготовленное.</p>
        </section>`;
    }

    const rating = Number(latestEntry.rating || 0);
    const fill = Math.max(0, Math.min(100, rating * 20));

    return `
      <section class="profile-rating-card">
        <div class="profile-rating-card__head">
          <span>Оценка блюда</span>
          <small>${rating ? "сохранена" : "ещё не оценено"}</small>
        </div>
        <button class="profile-rating-stars" type="button" data-action="rating-pick" data-recipe-id="${escapeHTML(recipe.id)}" data-entry-id="${escapeHTML(latestEntry.id)}" style="--rating-fill: ${fill}%;" aria-label="Оценить блюдо">
          <span class="profile-rating-stars__base" aria-hidden="true">★★★★★</span>
          <span class="profile-rating-stars__fill" aria-hidden="true">★★★★★</span>
        </button>
      </section>`;
  }

  function saveCookedRatingFromStars(target, event) {
    const entryId = target.dataset.entryId;
    const rect = target.getBoundingClientRect();
    if (!rect.width) return;

    const raw = ((event.clientX - rect.left) / rect.width) * 5;
    const rating = Math.max(0.5, Math.min(5, Math.round(raw * 2) / 2));
    saveCookedRating(entryId, rating);
  }


  function cookedRatingTemplate(recipe, entryId) {
    const values = [5, 4.5, 4, 3.5, 3, 2.5, 2, 1.5, 1, 0.5];
    return profileScreenShell(`
      <header class="profile-head profile-head--compact">
        <button class="recipe-screen__round profile-back" type="button" data-action="profile-recipe" data-recipe-id="${escapeHTML(recipe.id)}" aria-label="К статистике блюда">
          <svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M15.2 5.4 8.6 12l6.6 6.6"/></svg>
        </button>
        <div>
          <span>Подано к столу</span>
          <h2>${escapeHTML(recipe.title)}</h2>
          <p>Блюдо сохранено в профиль. Можно поставить оценку.</p>
        </div>
      </header>

      <section class="rating-card">
        <h3>Оцени блюдо</h3>
        <div class="rating-grid" aria-label="Оценка блюда">
          ${values.map(value => `
            <button type="button" data-action="rating-save" data-recipe-id="${escapeHTML(recipe.id)}" data-entry-id="${escapeHTML(entryId)}" data-rating="${value}">
              <span>★</span>${formatRating(value)}
            </button>`).join("")}
        </div>
        <button class="rating-skip" type="button" data-action="rating-skip" data-recipe-id="${escapeHTML(recipe.id)}">Пропустить</button>
      </section>
    `, "Оценка блюда");
  }

  function saveCookedRating(entryId, rating) {
    const entry = state.cookedHistory.find(item => item.id === entryId);
    if (!entry) return;

    entry.rating = Math.max(0.5, Math.min(5, Math.round(Number(rating) * 2) / 2));
    saveCookedHistory();
  ...
  function addRecipeToCartFromScreen(recipeId) {
    ...
    showToast("Уже в корзине", "neutral");
  }

  function normalizeCookedHistory(value) {
    if (!Array.isArray(value)) return [];

    return value
      .filter(entry => entry && getRecipe(entry.recipeId))
      .map(entry => ({
        id: String(entry.id || `${entry.cookedAt || Date.now()}-${entry.recipeId}`),
        recipeId: String(entry.recipeId),
        cookedAt: entry.cookedAt || new Date().toISOString(),
        portions: Number(entry.portions || 1),
        rating: Number(entry.rating) > 0 ? Math.max(0.5, Math.min(5, Math.round(Number(entry.rating) * 2) / 2)) : null
      }))
      .sort((a, b) => new Date(b.cookedAt) - new Date(a.cookedAt));
  }

  function saveCookedHistory() {
    state.cookedHistory = normalizeCookedHistory(state.cookedHistory);
    saveJson(STORAGE_COOKED_HISTORY, state.cookedHistory);
  }

  function getCookedHistory(recipeId = null) {
    const list = state.cookedHistory.filter(entry => getRecipe(entry.recipeId));
    const filtered = recipeId ? list.filter(entry => entry.recipeId === recipeId) : list;
    return filtered.sort((a, b) => new Date(b.cookedAt) - new Date(a.cookedAt));
  }

  function getCookedRecipeStats(recipeId) {
    const entries = getCookedHistory(recipeId);
    const avg = getAverageRating(entries);
    return {
      count: entries.length,
      avg,
      lastAt: entries[0]?.cookedAt || null
    };
  }

  function getAverageRating(entries) {
    const rated = entries.map(entry => Number(entry.rating)).filter(value => value > 0);
    if (!rated.length) return null;
    return rated.reduce((sum, value) => sum + value, 0) / rated.length;
  }

  function getCookedCountByCategory(categoryId) {
    if (categoryId === "all") return getCookedHistory().length;
    return getCookedHistory().filter(entry => getRecipe(entry.recipeId)?.category === categoryId).length;
  }

  function getCookedRecipesByCategory(categoryId) {
    const entries = getCookedHistory().filter(entry => {
      const recipe = getRecipe(entry.recipeId);
      return recipe && (categoryId === "all" || recipe.category === categoryId);
    });
  ...
  function getRecentCookedRecipes(limit = 5) {
    const seen = new Set();
    return getCookedHistory()
      .map(entry => getRecipe(entry.recipeId))
      .filter(recipe => {
        if (!recipe || seen.has(recipe.id)) return false;
        seen.add(recipe.id);
  ...
  function getProfileRecommendations(limit = 3) {
    const history = getCookedHistory();
    const recentIds = new Set(history.slice(0, 8).map(entry => entry.recipeId));
    const ratingByRecipe = new Map();

    getRecipes().forEach(recipe => {
      const stats = getCookedRecipeStats(recipe.id);
      ratingByRecipe.set(recipe.id, stats.avg || 0);
    });

    return compactRecipeVariants(getRecipes())
      .map((recipe, index) => {
        const rating = ratingByRecipe.get(recipe.id) || 0;
        const cooked = getCookedRecipeStats(recipe.id).count;
        const score =
          (rating ? rating * 2 : 4)
          + (cooked ? 0 : 2)
          - (recentIds.has(recipe.id) ? 5 : 0)
          - index * 0.001;
        return { recipe, score };
      })
  ...
  function formatRating(value) {
    ...
    return formatNumber(Math.round(Number(value) * 2) / 2);
  }
  ...
  function handleRecipeScreenClick(event) {
    ...
    if (actionName === "add-to-cart") {
      event.preventDefault();
      addRecipeToCartFromScreen(recipeId);
      return;
    }
    ...
    if (actionName === "rating-pick") {
      event.preventDefault();
      saveCookedRatingFromStars(action, event);
      return;
    }

    if (actionName === "rating-save") {
      event.preventDefault();
      saveCookedRating(action.dataset.entryId, Number(action.dataset.rating || 0));
      return;
    }

    if (actionName === "rating-skip") {
      event.preventDefault();
      openProfileRecipeStats(recipeId);
      return;
    }
    ...
    if (actionName === "profile-home") {
      event.preventDefault();
      openProfileScreen();
      return;
    }

    if (actionName === "profile-settings") {
      event.preventDefault();
      showToast("Настройки профиля добавим позже", "neutral");
      return;
    }

    if (actionName === "profile-category") {
      event.preventDefault();
      openProfileCategory(action.dataset.categoryId || "all");
      return;
    }

    if (actionName === "profile-recipe") {
      event.preventDefault();
      openProfileRecipeStats(recipeId);
      return;
    }

    if (actionName === "profile-open-recipe") {
      event.preventDefault();
      openRecipeScreen(recipeId);
      return;
    }
    ...
    if (actionName === "page-note") {
      event.preventDefault();
      showToast("Заметки добавим позже", "neutral");
      return;
    }
  }
  ...
  function recipeCookingScreenTemplate(recipe) {
    ...
    return `
      <article class="recipe-screen__panel recipe-cooking-screen" role="dialog" aria-modal="true" aria-label="Способ приготовления: ${escapeHTML(recipe.title)}">
        ...
        <button class="recipe-screen-note recipe-cooking-note recipe-served-btn" type="button" data-action="served" data-recipe-id="${escapeHTML(recipe.id)}">
          <span>
            <strong>Подано к столу</strong>
            <small>Сохранить приготовление в профиль</small>
          </span>
          <b aria-hidden="true">✓</b>
        </button>
      </article>`;
    ...
    return `
      <article class="recipe-screen__panel" role="dialog" aria-modal="true" aria-label="${escapeHTML(recipe.title)}">
        ...
        <div class="recipe-screen__body">
          ...
          <button class="recipe-screen-cart-btn ${state.selectedIds.includes(recipe.id) ? "selected" : ""}" type="button" data-action="add-to-cart" data-recipe-id="${escapeHTML(recipe.id)}">
            <span aria-hidden="true">${cartPlusIconMarkup()}</span>
            <strong>${state.selectedIds.includes(recipe.id) ? "Уже в корзине" : "Добавить в корзину"}</strong>
          </button>
  ...
  function cartPlusIconMarkup() {
    return `
      <svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
        <path d="M6.3 7.6h13.1l-1.35 7.65a2 2 0 0 1-1.95 1.65H9.25a2 2 0 0 1-1.95-1.65L6.3 7.6Z"/>
        <path d="M5 7.6h15.4"/>
        <path d="M8.7 7.6a3.3 3.3 0 0 1 6.6 0"/>
        <path d="M12.7 10.8v3.5M10.95 12.55h3.5"/>
      </svg>`;
  }
  ...
  function updateRecipeScreenSelectionUI(recipeId) {
    ...
    const cartBtn = state.refs.recipeScreen?.querySelector(`[data-action="add-to-cart"][data-recipe-id="${cssEscape(recipeId)}"]`);
    if (!cartBtn) return;

    const selected = state.selectedIds.includes(recipeId);
    cartBtn.classList.toggle("selected", selected);
    const label = cartBtn.querySelector("strong");
    if (label) label.textContent = selected ? "Уже в корзине" : "Добавить в корзину";
  }
