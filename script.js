/* Portionly — static app logic, dashboard menu + recipe dropdown cards + cart planning v79 */
(() => {
  "use strict";
  ...
  function renderCategories() {
    ...
    state.refs.categoryTabs.innerHTML = categoryItems.map(category => `
      <button class="category-tab category-tile ${category.id === state.activeCategory && !state.dashboardOpen ? "active" : ""}" type="button" data-category="${escapeHTML(category.id)}" ${category.disabled ? 'disabled aria-disabled="true"' : ""}>
        ...
        <span class="category-tile__content">
          ...
          <small>${category.disabled ? "пока пусто" : `${category.count} ${plural(category.count, "блюдо", "блюда", "блюд")}`}</small>
        </span>
      </button>
    `).join("");
    ...
    state.refs.categoryTabs.querySelectorAll("[data-category]").forEach(button => {
      button.addEventListener("click", () => {
        ...
        renderRecipes({ animate: true });
        requestAnimationFrame(() => {
          state.refs.recipeListHead?.scrollIntoView?.({ behavior: "smooth", block: "start" });
        });
      });
    });
