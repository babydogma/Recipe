...
(() => {
  ...
  function getRecipePageRoot(recipeId, card = null) {
    ...
    return parent.querySelector(`[data-page-recipe-id="${cssEscape(recipeId)}"]`) || parent;
  }

  function showRecipeVariantStep(rootId, step) {
    const card = getRecipeCardElement(rootId);
    const pages = card?.querySelector(".recipe-card-pages");
    if (!pages) return;

    const pageCount = pages.querySelectorAll(".recipe-card-page").length;
    if (pageCount < 2) return;

    const current = getRecipeCardPageIndex(pages);
    const next = clamp(current + step, 0, pageCount - 1);
  ...
  function handleCardsGridClick(event) {
    ...
    toggleRecipe(page?.dataset.pageRecipeId || card.dataset.cardId);
  }
  ...
  function getRecipeCardPageIndex(pages) {
    ...
    const maxIndex = Math.max(0, pages.querySelectorAll(".recipe-card-page").length - 1);
    return clamp(Math.round(pages.scrollLeft / width), 0, maxIndex);
  }
  ...
  function syncRecipeCardSize(pages) {
    ...
    const pagesList = Array.from(pages.querySelectorAll(".recipe-card-page"));
    const activePage = pagesList[getRecipeCardPageIndex(pages)] || pages.querySelector(".recipe-card-page--cover");
    ...
    const fixedHeight = Math.ceil(activePage.scrollHeight);
    if (fixedHeight > 0) {
  ...
  function syncRecipeCardSizeAround(node) {
    const pages = node?.closest?.(".recipe-card-pages");
    if (!pages) return;
    syncRecipeCardSize(pages);
    syncRecipeCardPager(pages);
  }
  ...
  function pulseRecipeCardSizeSync(node) {
    ...
    syncRecipeCardSize(pages);
    syncRecipeCardPager(pages);
    requestAnimationFrame(() => syncRecipeCardSize(pages));
    window.setTimeout(() => syncRecipeCardSize(pages), 80);
    window.setTimeout(() => syncRecipeCardSize(pages), 220);
    window.setTimeout(() => syncRecipeCardSize(pages), 460);
  }
  ...
  function syncRecipeCardState(card, recipeId) {
    ...
    const hasExpanded = recipes.some(recipe => state.expandedIds.has(recipe.id));

    card.classList.toggle("selected", hasSelected);
    card.classList.toggle("expanded", hasExpanded);

    recipes.forEach(recipe => {
      syncRecipeCardPageState(card, recipe.id);
    });
  ...
  function recipeVariantCardTemplate(rootRecipe, variantRecipes) {
    ...
    const expanded = variantRecipes.some(recipe => state.expandedIds.has(recipe.id));
    const image = getRecipeImage(rootRecipe);
    const variantIds = variantRecipes.map(recipe => recipe.id).join(" ");

    return `
      <article class="recipe-card recipe-card--variants is-first-variant ${selected ? "selected" : ""} ${expanded ? "expanded" : ""} ${image ? "" : "missing-image"}" data-card-id="${escapeHTML(rootRecipe.id)}" data-variant-ids="${escapeHTML(variantIds)}" data-variant-index="0">
        <div class="recipe-card-pages" data-variant-pages="${escapeHTML(rootRecipe.id)}" aria-label="Вариации рецепта">
          ${variantRecipes.map((recipe, index) => `
            <section class="recipe-card-page recipe-card-page--cover" data-page-recipe-id="${escapeHTML(recipe.id)}">
              ${recipeCardContentTemplate(recipe, { rootId: rootRecipe.id, count: variantRecipes.length, index })}
            </section>`).join("")}
        </div>
  ...
  function toggleExpanded(recipeId) {
    ...
    card.classList.toggle("expanded", getRecipeCardRecipes(recipeId).some(recipe => state.expandedIds.has(recipe.id)));
  ...
  function replaceRecipeCard(recipeId) {
    ...
    const currentPages = currentCard.querySelector(".recipe-card-pages");
    const currentVariantIndex = currentPages ? getRecipeCardPageIndex(currentPages) : 0;
    ...
    if (nextCard) {
      ...
      const nextPages = nextCard.querySelector(".recipe-card-pages");
      if (nextPages) {
        const pageCount = nextPages.querySelectorAll(".recipe-card-page").length;
        const nextIndex = clamp(currentVariantIndex, 0, Math.max(0, pageCount - 1));
        nextPages.scrollLeft = nextIndex * nextPages.clientWidth;
        syncRecipeCardPager(nextPages);
        syncRecipeCardSize(nextPages);
        requestAnimationFrame(() => {
          nextPages.scrollLeft = nextIndex * nextPages.clientWidth;
          syncRecipeCardPager(nextPages);
          syncRecipeCardSize(nextPages);
