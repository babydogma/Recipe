...
(() => {
  ...
  function basketCompactSummaryTemplate(totals) {
    return `
      <section class="basket-total-chips" aria-label="КБЖУ корзины">
        <span>${escapeHTML(formatNumber(totals.nutrition.kcal))} ккал</span>
        <span>Б ${escapeHTML(formatNumber(totals.nutrition.protein))}</span>
        <span>Ж ${escapeHTML(formatNumber(totals.nutrition.fat))}</span>
        <span>У ${escapeHTML(formatNumber(totals.nutrition.carbs))}</span>
      </section>
    `;
  }
      ...
      <section class="basket-dishes" aria-label="Блюда в корзине">
        <header class="basket-section-head">
          <div>
            ...
            <p>Раскрой блюдо и отметь продукты, которые уже есть дома.</p>
          </div>
        </header>
  ...
  function basketDishTemplate(recipe) {
    ...
    const nutrition = getScaledNutrition(recipe, portions);

    return `
      <article class="basket-dish ${expanded ? "is-open" : ""}">
        <div class="basket-dish__main">
          <button class="basket-dish__toggle" type="button" data-basket-dish-toggle="${escapeHTML(recipe.id)}" aria-expanded="${expanded ? "true" : "false"}">
            ...
            <span class="basket-dish__copy">
              ...
              <small>${portions} ${plural(portions, "порция", "порции", "порций")} · ${formatNumber(nutrition.kcal)} ккал</small>
            </span>
            <em aria-hidden="true">›</em>
          </button>
          <button class="basket-dish__remove" type="button" data-remove-id="${escapeHTML(recipe.id)}" aria-label="Убрать ${escapeHTML(recipe.title)} из корзины">×</button>
        </div>
        <div class="basket-dish__details" ${expanded ? "" : "hidden"}>
          <div class="basket-dish__details-head">
            <span>Отметь, что уже есть дома</span>
            <button type="button" data-copy-recipe-id="${escapeHTML(recipe.id)}">Скопировать недостающее</button>
          </div>
          <div class="basket-dish__ingredients">
            ${getScaledIngredients(recipe, portions).map(item => dishIngredientRowTemplate(recipe.id, item)).join("")}
          </div>
        </div>
      </article>
    `;
  ...
  function dishIngredientRowTemplate(recipeId, item) {
    ...
    const label = item.amount === null || item.amount === undefined
      ? normalized.unit
    ...
    return `
      <div class="basket-dish-ingredient ${checked ? "pantry-active" : ""}">
        <label class="pantry-checkbox" aria-label="Есть дома: ${escapeHTML(normalized.name)}">
          <input type="checkbox" data-dish-pantry-toggle data-dish-pantry-key="${escapeHTML(key)}" ${checked ? "checked" : ""}>
  ...
  function calculateTotals(selectedRecipes) {
    ...
    selectedRecipes.forEach(recipe => {
      ...
      getScaledIngredients(recipe, targetPortions).forEach(ingredient => {
        ...
        if (isDishIngredientPantryChecked(recipe.id, ingredient)) return;
  ...
  function basketFlatProductsTemplate(totals) {
    ...
    return products || `<div class="modal-empty">Нет ингредиентов для расчёта.</div>`;
  }
  ...
  function isDishIngredientPantryChecked(recipeId, item) {
    return state.pantryMap[getDishPantryKey(recipeId, item)] === true;
  }
  ...
  function normalizePantryMap(next) {
    ...
    Object.entries(next).forEach(([key, value]) => {
      ...
      else if (value && typeof value === "object" && value.checked === true) result[key] = true;
    });
  ...
  function handleDishPantryChange(event) {
    ...
    if (toggle.checked) state.pantryMap[key] = true;
    else delete state.pantryMap[key];
  ...
  function copyRecipeIngredientsText(recipeId, button) {
    ...
    getScaledIngredients(recipe, targetPortions)
      .filter(item => !isDishIngredientPantryChecked(recipe.id, item))
      .forEach(item => {
        const normalized = normalizeIngredientItem(item);
        const amount = item.amount === null || item.amount === undefined
          ? normalized.unit
          : formatAmount(item.amount, normalized.unit);
        lines.push(`- ${normalized.name}: ${amount}`);
      });
  ...
  function showCopySuccess() {
    if (state.refs.copyBtn) {
      ...
      setTimeout(() => { state.refs.copyBtn.textContent = "Скопировать список"; }, 1200);
    }
