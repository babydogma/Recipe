...
(() => {
  ...
  function renderCategories() {
    ...
    state.refs.categoryTabs.querySelectorAll("[data-category]").forEach(button => {
      button.addEventListener("click", () => {
        ...
        state.selectedOnly = false;
        warmRecipeImagesForCategory(state.activeCategory);
        state.search = "";
        state.searchOpen = false;
        if (state.refs.searchInput) state.refs.searchInput.value = "";
        renderCategories();
  ...
  function renderRecipes(options = {}) {
    ...
    const applyRecipeVisibility = () => {
      ...
      warmRecipeImagesForRecipes(visibleRecipes, { limit: 10 });
  ...
  function preloadRecipeImages() {
    const urls = Array.from(new Set(getRecipes().map(getRecipeImage).filter(Boolean)));
    if (!urls.length) return;

    preloadImageUrls(urls.slice(0, 12));

    const rest = urls.slice(12);
    if (!rest.length) return;

    let index = 0;
    const loadNextBatch = () => {
      preloadImageUrls(rest.slice(index, index + 4));
      index += 4;
  ...
  function warmRecipeImagesForCategory(categoryId) {
    warmRecipeImagesForRecipes(getRecipesForCategory(categoryId), { limit: 12 });
  }

  function warmRecipeImagesForRecipes(recipes, options = {}) {
    const limit = Number.isFinite(options.limit) ? Math.max(1, options.limit) : 8;
    const urls = [];
    const seen = new Set();

    recipes.forEach(recipe => {
      getRecipeCardRecipes(recipe.id).forEach(item => {
        const image = getRecipeImage(item);
    ...
    preloadImageUrls(urls.slice(0, limit));
  }

  function getRecipesForCategory(categoryId) {
    ...
    if (id === "favorites") {
      ...
      list = list.filter(recipe => recipe.category === id);
    }
  ...
  function warmImage(url) {
    if (!url || state.imagePreloadCache.has(url)) return;
    ...
    state.imagePreloadCache.set(url, img);
  }
