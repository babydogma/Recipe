/* Portionly — static app logic, swipe cards + cart planning v75 */
(() => {
  "use strict";
  ...
  function bindStaticEvents() {
    ...
    r.cardsGrid?.addEventListener("scroll", handleRecipeCardPagesScroll, { passive: true, capture: true });
    r.cardsGrid?.addEventListener("load", event => {
      if (event.target?.matches?.(".recipe-hero img, .recipe-page-bg")) scheduleRecipeCardHeightSync();
    }, true);
  ...
  function handleRecipeCardPointerDown(event) {
    ...
    state.cardGesture = {
      ...
      lastX: event.clientX,
      lastY: event.clientY,
      startPage: getRecipeCardPageIndex(pages),
      moved: false
    };
  ...
  function handleRecipeCardPointerEnd() {
    ...
    if (gesture?.card && gesture.moved) {
      const dx = (gesture.lastX ?? gesture.startX) - gesture.startX;
      const direction = dx < -26 ? 1 : dx > 26 ? -1 : 0;
      const currentPage = getRecipeCardPageIndex(gesture.pages);
      const targetPage = direction
        ? clamp(gesture.startPage + direction, 0, 2)
        : clamp(currentPage, Math.max(0, gesture.startPage - 1), Math.min(2, gesture.startPage + 1));

      window.setTimeout(() => snapRecipeCardToPage(gesture.pages, targetPage), 40);
      window.setTimeout(() => {
  ...
  function snapRecipeCardToPage(pages, page) {
    if (!pages?.classList?.contains("recipe-card-pages")) return;
    const width = Math.max(1, pages.clientWidth);
    pages.scrollTo({ left: width * clamp(page, 0, 2), behavior: "smooth" });
    syncRecipeCardPager(pages);
  }
  ...
  function recipeCardTemplate(recipe) {
    ...
    const backdrop = image
      ? `<img class="recipe-page-bg" src="${escapeHTML(image)}" alt="" aria-hidden="true" loading="lazy" onerror="window.handleRecipeImageError(this)">`
      : `<div class="recipe-page-bg recipe-page-bg--empty" aria-hidden="true"></div>`;

    return `
      <article class="recipe-card ${selected ? "selected" : ""} ${image ? "" : "missing-image"}" data-card-id="${escapeHTML(recipe.id)}">
        <div class="recipe-card-pages" aria-label="Карточка рецепта с ингредиентами и способом приготовления">
          <section class="recipe-card-page recipe-card-page--cover" aria-label="Основная информация">
          ...
          <section class="recipe-card-page recipe-card-page--info" data-recipe-info-page aria-label="Ингредиенты">
            ${backdrop}
            <div class="recipe-page-shade"></div>
            <div class="recipe-info-panel">
              <div class="recipe-info-head">
                <span>Ингредиенты</span>
                <small>на 1 порцию</small>
              </div>
              <div class="ingredients-grid recipe-info-list">
                ${ingredientRowsTemplate(recipe.ingredients)}
              </div>
            </div>
          </section>

          <section class="recipe-card-page recipe-card-page--info" data-recipe-info-page aria-label="Способ приготовления">
            ${backdrop}
            <div class="recipe-page-shade"></div>
            <div class="recipe-info-panel recipe-info-panel--steps">
              <div class="recipe-info-head">
                <span>Как готовить</span>
                <small>${steps.length} ${plural(steps.length, "шаг", "шага", "шагов")}</small>
              </div>
              <ol class="steps-list recipe-info-steps">
        ...
        <div class="recipe-card-dots" aria-hidden="true">
          <span class="active" data-recipe-dot></span>
          <span data-recipe-dot></span>
          <span data-recipe-dot></span>
        </div>
      </article>`;
  }
