/* Portionly — static app logic, swipe cards + cart planning v73 */
(() => {
  "use strict";
  ...
  function handleRecipeCardPagesScroll(event) {
    ...
    const page = clamp(Math.round(pages.scrollLeft / width), 0, 2);
  ...
  function recipeCardTemplate(recipe) {
    ...
    return `
      <article class="recipe-card ${selected ? "selected" : ""} ${image ? "" : "missing-image"}" data-card-id="${escapeHTML(recipe.id)}">
        <div class="recipe-card-pages" aria-label="Карточка рецепта: свайпни влево, чтобы посмотреть ингредиенты и способ приготовления">
          <section class="recipe-card-page recipe-card-page--cover" aria-label="Основная информация">
            ...
            <div class="recipe-card-body">
              ...
              <div class="recipe-swipe-hint" aria-hidden="true">
                <span>Свайп</span>
                <b>›</b>
              </div>
            </div>
