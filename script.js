/* Portionly — static app logic, per-recipe portions v12 */
(() => {
  "use strict";

  const STORAGE_SELECTED = "portionly:selected:v12";
  const STORAGE_NOTES = "portionly:notes:v12";
  const STORAGE_RECIPE_SETTINGS = "portionly:recipe-settings:v12";
  ...
  function toggleExpanded(recipeId) {
    ...
    // Если закрываем карточку с открытым редактором заметки, один раз пересобираем только её,
    // чтобы редактор точно исчез. Обычное раскрытие/скрытие идёт без перерендера и без мигания фото.
  ...
  function updateSelectionUI() {
    ...
    if (r.basketBadge) r.basketBadge.textContent = count;

    if (r.basketFab) {
      r.basketFab.hidden = !hasSelection;
      r.basketFab.classList.toggle("visible", hasSelection);
    }
    ...
    const dockTitle = document.getElementById("selectionDockTitle");
    const dockMeta = document.getElementById("selectionDockMeta");
    if (dockTitle) dockTitle.textContent = `${count} ${plural(count, "блюдо выбрано", "блюда выбрано", "блюд выбрано")}`;
    if (dockMeta) dockMeta.textContent = `${totalPortions} ${plural(totalPortions, "порция", "порции", "порций")} суммарно · нажми для итогов`;
  }
