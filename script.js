...
function recipeCardTemplate(recipe) {
  ...
  const categoryLabel = getCardCategoryLabel(category);
  const cardImage = recipe.heroImage || recipe.image;
    ...
    <article class="recipe-card ${selected ? "selected" : ""}">
      <div class="recipe-card-media">
        <button class="recipe-image-button" type="button" data-open-recipe="${recipe.id}" aria-label="Открыть ${escapeHTML(recipe.title)}">
          <img src="${cardImage}" alt="${escapeHTML(recipe.title)}" loading="lazy">
          <span class="image-vignette"></span>
          <span class="image-open-hint">Открыть рецепт</span>
        </button>
      ...
      <div class="recipe-card-body">
        ...
        <p class="recipe-card-meta">${escapeHTML(recipe.meta || `${recipe.portions} ${plural(recipe.portions, "порция", "порции", "порций")}`)}</p>

        <section class="recipe-glass-panel" aria-label="Данные рецепта">
          <div class="panel-section-head">
            <span class="panel-icon">▧</span>
            <div>
              <strong>Ингредиенты</strong>
              <small>${recipe.portions} ${plural(recipe.portions, "порция", "порции", "порций")}</small>
            </div>
          </div>
          ...
          <div class="card-ingredients">
            ${recipe.ingredients.map(ingredient => `
              <div class="card-ingredient">
                ...
                <b>${ingredient.amount === null ? escapeHTML(ingredient.unit) : formatAmount(ingredient.amount, ingredient.unit)}</b>
              </div>
            `).join("")}
          </div>
          ...
          <div class="panel-section-head steps-title">
            <span class="panel-icon">▤</span>
            <div>
...
function getCardCategoryLabel(category) {
  if (!category) return "БЛЮДО";
  if (category.id === "breakfast") return "ЗАВТРАК";
  if (category.id === "main") return "ОБЕД";
...
function renderBasketModal() {
  ...
  recipeSummaryList.innerHTML = selectedRecipes.map(recipe => `
    <article class="recipe-summary">
      <img src="${recipe.heroImage || recipe.image}" alt="${escapeHTML(recipe.title)}">
      <div>
