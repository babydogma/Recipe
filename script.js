...
(() => {
      ...
      <section class="basket-dishes" aria-label="Блюда в корзине">
        <header class="basket-section-head">
          <div>
            ...
            <p>Раскрой блюдо, если нужен список именно по нему.</p>
          </div>
        </header>
      ...
      <article class="basket-dish ${expanded ? "is-open" : ""}">
        ...
        <div class="basket-dish__details" ${expanded ? "" : "hidden"}>
          <div class="basket-dish__details-head">
            <span>Ингредиенты блюда</span>
            <button type="button" data-copy-recipe-id="${escapeHTML(recipe.id)}">Скопировать</button>
          </div>
          <div class="basket-dish__ingredients">
            ${getScaledIngredients(recipe, portions).map(dishIngredientRowTemplate).join("")}
          </div>
        </div>
      </article>
    `;
  ...
  function dishIngredientRowTemplate(item) {
    ...
    return `
      <div class="basket-dish-ingredient">
  ...
  function copyRecipeIngredientsText(recipeId, button) {
    ...
    getScaledIngredients(recipe, targetPortions).forEach(item => {
      const normalized = normalizeIngredientItem(item);
  ...
  function showDishCopySuccess(button) {
    ...
    showToast("Ингредиенты скопированы", "success");
  }
  ...
  function showCopySuccess() {
    if (state.refs.copyBtn) {
      ...
      setTimeout(() => { state.refs.copyBtn.textContent = "Скопировать всё"; }, 1200);
    }
