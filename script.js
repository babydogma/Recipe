/* Portionly — static app logic, dashboard menu + recipe dropdown cards + cart planning v83 */
(() => {
  "use strict";
  ...
  const DASHBOARD_MEDIA = {
    hero: { image: "assets/chicken-cabbage-salad.jpg", position: "center 45%" },
    all: { image: "assets/omelet-cheese-tomato-toast.jpg", position: "center 45%" },
    favorites: { image: "assets/cottage-cheese-berry-dessert.jpg", position: "center 48%" },
    breakfast: { image: "assets/cottage-cheese-banana-berries-honey.jpg", position: "center 46%" },
    main: { image: "assets/chicken-cabbage-salad.jpg", position: "center 44%" },
    dinner: { image: "assets/fish-potato-cauliflower-puree.jpg", position: "center 45%" },
    dessert: { image: "assets/cottage-cheese-berry-dessert.jpg", position: "center 48%" },
    shopping: { image: "assets/chicken-lavash-vegetables-yogurt.jpg", position: "center 45%" }
  };
  ...
  function bindStaticEvents() {
    ...
    r.dashboardFilterBtn?.addEventListener("click", () => {
      showToast("Фильтры уже вынесены в быстрые разделы ниже", "neutral");
    });
  ...
  function renderCategories() {
    ...
    const categoryItems = categories.map(category => ({
      ...category,
      count: getCategoryRecipeCount(category.id, recipes),
      disabled: false
    }));
    ...
    const allItem = categoryItems.find(category => category.id === "all") || {
      id: "all",
      title: "Все блюда",
    ...
    const breakfastItem = categoryItems.find(category => category.id === "breakfast");
    const lunchItem = categoryItems.find(category => category.id === "main");
    const dinnerItem = categoryItems.find(category => category.id === "dinner");
    const dessertItem = categoryItems.find(category => category.id === "dessert");
    const mealItems = [breakfastItem, lunchItem, dinnerItem, dessertItem].filter(Boolean);

    state.refs.categoryTabs.innerHTML = `
      <section class="dashboard-hero-card" ${dashboardImageStyle("hero")}>
        <div class="dashboard-hero-card__content">
          <span>Идея на сегодня</span>
          <h2>Что приготовить сегодня?</h2>
          <p>3 быстрые идеи из твоего каталога</p>
          <button class="dashboard-hero-card__button" type="button" data-dashboard-filter="today">
            <strong>Посмотреть идеи</strong>
            <b aria-hidden="true">→</b>
          </button>
        </div>
        <div class="dashboard-hero-card__dots" aria-hidden="true"><i></i><i></i><i></i></div>
      </section>

      <section class="dashboard-section dashboard-section--quick">
        <h3>Быстрый доступ</h3>
        <div class="dashboard-quick-grid">
          ${dashboardWideTileTemplate(allItem, "all", "grid")}
          ${dashboardWideTileTemplate(favoriteItem, "favorites", "heart")}
        </div>
      </section>

      <section class="dashboard-section dashboard-section--meals">
        <h3>Приёмы пищи</h3>
        <div class="dashboard-meal-grid">
          ${mealItems.map(item => dashboardMealTileTemplate(item)).join("")}
        </div>
      </section>

      <section class="dashboard-section dashboard-section--planning">
        <h3>Планирование и покупки</h3>
        <div class="dashboard-planning-grid">
          <button class="dashboard-plan-card dashboard-plan-card--shopping" type="button" data-dashboard-action="basket" ${dashboardImageStyle("shopping")}>
            <span class="dashboard-plan-card__icon" aria-hidden="true">${dashboardIconTemplate("bag")}</span>
            <strong>Список покупок</strong>
            <em>${getBasketProductsCount()} ${plural(getBasketProductsCount(), "продукт", "продукта", "продуктов")}</em>
            <small>Из выбранных блюд</small>
          </button>
          <div class="dashboard-plan-stack">
            <button class="dashboard-filter-card" type="button" data-dashboard-filter="quick">
              <span aria-hidden="true">${dashboardIconTemplate("clock")}</span>
              <strong>Быстрые блюда</strong>
              <small>до 20 минут</small>
              <b aria-hidden="true">→</b>
            </button>
            <button class="dashboard-filter-card" type="button" data-dashboard-filter="lowcal">
              <span aria-hidden="true">${dashboardIconTemplate("leaf")}</span>
    ...
    state.refs.categoryTabs.querySelectorAll("[data-category]").forEach(button => {
      button.addEventListener("click", () => {
        ...
        openRecipeCategory(button.dataset.category);
      });
    });

    state.refs.categoryTabs.querySelectorAll("[data-dashboard-filter]").forEach(button => {
      button.addEventListener("click", () => {
        openDashboardFilter(button.dataset.dashboardFilter);
      });
    });

    state.refs.categoryTabs.querySelectorAll("[data-dashboard-action]").forEach(button => {
      button.addEventListener("click", () => {
        if (button.dataset.dashboardAction === "basket") openBasketModal();
      });
    });
  ...
  function getCategoryRecipeCount(categoryId, recipes = getRecipes()) {
    if (categoryId === "all") return recipes.length;
    return recipes.filter(recipe => recipe.category === categoryId).length;
  }

  function dashboardImageStyle(key) {
    const media = DASHBOARD_MEDIA[key];
    if (!media?.image) return "";
    const position = media.position || "center";
    return `style="--dashboard-image: url('${escapeHTML(media.image)}'); --dashboard-position: ${escapeHTML(position)};"`;
  }

  function dashboardWideTileTemplate(category, mediaKey, iconKey) {
    const disabled = Boolean(category.disabled);
    return `
      <button class="dashboard-tile dashboard-tile--wide ${disabled ? "is-disabled" : ""}" type="button" data-category="${escapeHTML(category.id)}" ${disabled ? 'disabled aria-disabled="true"' : ""} ${dashboardImageStyle(mediaKey)}>
        <span class="dashboard-tile__icon" aria-hidden="true">${dashboardIconTemplate(iconKey)}</span>
        <span class="dashboard-tile__body">
          <strong>${escapeHTML(category.title)}</strong>
          <em>${category.count} ${plural(category.count, "блюдо", "блюда", "блюд")}</em>
        </span>
      </button>
    `;
  }

  function dashboardMealTileTemplate(category) {
    ...
    return `
      <button class="dashboard-tile dashboard-tile--meal" type="button" data-category="${escapeHTML(category.id)}" ${dashboardImageStyle(category.id)}>
        <span class="dashboard-tile__icon" aria-hidden="true">${dashboardIconTemplate(iconByCategory[category.id] || "grid")}</span>
        <span class="dashboard-tile__body">
          <strong>${escapeHTML(category.title)}</strong>
          <em>${category.count} ${plural(category.count, "блюдо", "блюда", "блюд")}</em>
        </span>
  ...
  function openRecipeCategory(categoryId) {
  ...
  function openDashboardFilter(filterId) {
    state.dashboardOpen = false;
    state.activeCategory = "all";
    state.dashboardFilter = filterId || null;
    state.selectedOnly = false;
    state.search = "";
    state.searchOpen = false;
    if (state.refs.searchInput) state.refs.searchInput.value = "";
  ...
  function dashboardIconTemplate(icon) {
    const icons = {
      grid: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M5 5h5v5H5V5Zm9 0h5v5h-5V5ZM5 14h5v5H5v-5Zm9 0h5v5h-5v-5Z"/></svg>`,
      heart: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M12 20.2C8 17.5 5.4 14.7 5.4 11.4c0-2.4 1.7-4.1 3.9-4.1 1.2 0 2.2.5 2.7 1.4.5-.9 1.5-1.4 2.7-1.4 2.2 0 3.9 1.7 3.9 4.1 0 3.3-2.6 6.1-6.6 8.8Z"/></svg>`,
      sun: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M12 16.2a4.2 4.2 0 1 0 0-8.4 4.2 4.2 0 0 0 0 8.4Z"/><path d="M12 3.4v2M12 18.6v2M4.2 12h2M17.8 12h2M6.5 6.5l1.4 1.4M16.1 16.1l1.4 1.4M17.5 6.5l-1.4 1.4M7.9 16.1l-1.4 1.4"/></svg>`,
      "sun-small": `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M12 16a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z"/><path d="M12 4v2M12 18v2M4 12h2M18 12h2M6.7 6.7l1.4 1.4M15.9 15.9l1.4 1.4M17.3 6.7l-1.4 1.4M8.1 15.9l-1.4 1.4"/></svg>`,
      moon: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M18.2 16.7A7.2 7.2 0 0 1 8 6.5a7.7 7.7 0 1 0 10.2 10.2Z"/></svg>`,
      cupcake: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M6.8 10.2h10.4l-1.1 8.2H7.9l-1.1-8.2Z"/><path d="M5.7 10.2h12.6"/><path d="M8.4 10.2c.1-2 1.5-3.4 3.6-3.4s3.5 1.4 3.6 3.4"/><path d="M12 6.8V4.4"/></svg>`,
      bag: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M6.4 8.7h11.2l-.8 10.1H7.2L6.4 8.7Z"/><path d="M9 8.7c0-2.2 1.1-3.5 3-3.5s3 1.3 3 3.5"/></svg>`,
      clock: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Z"/><path d="M12 7.2V12l3.1 1.9"/></svg>`,
      leaf: `<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M19.4 4.8c-6.6.4-11.1 3.5-13.4 9.2 2.4 3.5 7.7 3.1 10.4-.4 1.7-2.2 2.5-5.1 3-8.8Z"/><path d="M6 17.2c2.2-4.3 5.2-7 9-8.1"/></svg>`
    };
  ...
  function getVisibleRecipes() {
    ...
    if (state.dashboardFilter === "quick") {
      list = list.filter(recipe => getCookTimeSortRange(recipe).max <= 20);
    } else if (state.dashboardFilter === "lowcal") {
      list = list.filter(recipe => Number(recipe?.nutrition?.kcal) <= 350);
    } else if (state.dashboardFilter === "today") {
      list = getDailyIdeaRecipes(list);
    }
  ...
  function getDailyIdeaRecipes(recipes) {
    ...
    for (let offset = 0; offset < sorted.length && ideas.length < 3; offset += 1) {
      ideas.push(sorted[(start + offset) % sorted.length]);
    }
  ...
  function updateRecipeListHeader(count, visible) {
    ...
    if (query) {
    ...
    } else if (state.dashboardFilter === "quick") {
      title = "Быстрые блюда";
      ...
      subtitle = `${count} ${plural(count, "идея", "идеи", "идей")} из каталога`;
