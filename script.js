/* Portionly — static app logic, Figma-style dashboard + recipe dropdown cards + cart planning v98 */
(() => {
  "use strict";
  ...
  function categoryFeaturedRecipeTemplate(recipe, config) {
    ...
    return `
      <section class="category-feature-section" aria-label="${escapeHTML(config.pickTitle)}">
        ...
        <article class="category-feature-card ${state.selectedIds.includes(recipe.id) ? "selected" : ""} ${expanded ? "expanded" : ""}" data-card-id="${escapeHTML(recipe.id)}" data-page-recipe-id="${escapeHTML(recipe.id)}">
          ...
          <div class="category-feature-card__body">
            ...
            <button class="category-feature-card__open ${expanded ? "open" : ""}" type="button" data-action="expand" data-recipe-id="${escapeHTML(recipe.id)}" aria-expanded="${expanded ? "true" : "false"}" aria-controls="recipe-details-${escapeHTML(recipe.id)}">
              <span>${expanded ? "Скрыть рецепт" : "Смотреть рецепт"}</span>
              <b aria-hidden="true">›</b>
            </button>
  ...
  function categoryMiniRecipeTemplate(recipe) {
    ...
    return `
      <article class="category-mini-card ${state.selectedIds.includes(recipe.id) ? "selected" : ""}" data-card-id="${escapeHTML(recipe.id)}" data-page-recipe-id="${escapeHTML(recipe.id)}">
