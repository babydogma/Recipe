/* Portionly — static app logic, per-recipe portions v9 */
(() => {
  "use strict";

  const STORAGE_SELECTED = "portionly:selected:v9";
  const STORAGE_NOTES = "portionly:notes:v9";
  const STORAGE_RECIPE_SETTINGS = "portionly:recipe-settings:v9";
        ...
        <div class="recipe-card-body">
          ...
          <div class="nutrition-block">
            <div class="nutrition-head">
              ...
              <small>Расчёт этой карточки: ${targetPortions} ${plural(targetPortions, "порция", "порции", "порций")}</small>
            </div>
            ...
            <div class="scaled-inline">Итого: ${formatNumber(scaledNutrition.kcal)} ккал · Б ${formatNumber(scaledNutrition.protein)} · Ж ${formatNumber(scaledNutrition.fat)} · У ${formatNumber(scaledNutrition.carbs)}</div>
          </div>
          ...
          <div class="recipe-details ${expanded ? "visible" : ""}">
            <section class="recipe-plan-card">
              ...
              <div class="recipe-plan-grid">
                ${recipeMiniStepperTemplate(recipe.id, "people", "Людей", plan.people, MIN_SETTING, MAX_PEOPLE)}
                ${recipeMiniStepperTemplate(recipe.id, "days", "Дней", plan.days, MIN_SETTING, MAX_DAYS)}
                <div class="plan-total">
                  <span>Итого</span>
                  <strong>${targetPortions} ${plural(targetPortions, "порция", "порции", "порций")}</strong>
                </div>
              </div>
            </section>
  ...
  function recipeMiniStepperTemplate(recipeId, key, label, value, min, max) {
    return `
      <div class="recipe-mini-stepper">
        <span>${escapeHTML(label)}</span>
        <div>
          <button type="button" data-action="recipe-step" data-recipe-id="${escapeHTML(recipeId)}" data-key="${escapeHTML(key)}" data-delta="-1" ${value <= min ? "disabled" : ""}>−</button>
          <strong>${value}</strong>
          <button type="button" data-action="recipe-step" data-recipe-id="${escapeHTML(recipeId)}" data-key="${escapeHTML(key)}" data-delta="1" ${value >= max ? "disabled" : ""}>+</button>
        </div>
      </div>
    `;
  }
  ...
  function updateRecipeSetting(recipeId, key, delta) {
    ...
    plan[key] = clamp(plan[key] + delta, MIN_SETTING, max);
