...
(() => {
  ...
  function showNextRecipeVariant(rootId) {
  ...
  function handleCardsGridClick(event) {
    ...
    if (action) {
      ...
      if (actionName === "variant-next") showNextRecipeVariant(action.dataset.rootId);
      return;
    }
  ...
  function recipeVariantCardTemplate(rootRecipe, variantRecipes) {
    ...
    return `
      <article class="recipe-card recipe-card--variants ${selected ? "selected" : ""} ${expanded ? "expanded" : ""} ${image ? "" : "missing-image"}" data-card-id="${escapeHTML(rootRecipe.id)}" data-variant-ids="${escapeHTML(variantIds)}">
        <div class="recipe-card-pages" data-variant-pages="${escapeHTML(rootRecipe.id)}" aria-label="Вариации рецепта">
          ${variantRecipes.map(recipe => `
            <section class="recipe-card-page recipe-card-page--cover" data-page-recipe-id="${escapeHTML(recipe.id)}">
              ${recipeCardContentTemplate(recipe)}
            </section>`).join("")}
        </div>
        <button class="recipe-variant-arrow" type="button" data-action="variant-next" data-root-id="${escapeHTML(rootRecipe.id)}" aria-label="Показать следующую вариацию рецепта">›</button>
        <div class="recipe-card-dots" aria-hidden="true">
          ${variantRecipes.map((_, index) => `<span data-recipe-dot class="${index === 0 ? "active" : ""}"></span>`).join("")}
        </div>
      </article>`;
  }

  function recipeCardContentTemplate(recipe) {
