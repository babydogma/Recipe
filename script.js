...
(() => {
  ...
  function renderCategories() {
    ...
    state.refs.categoryTabs.innerHTML = categoryItems.map(category => {
      const tileMedia = getCategoryTileMedia(category.id);
      const tileStyle = tileMedia
        ? ` style="--tile-image: url('${escapeHTML(tileMedia.image)}'); --tile-image-position: ${escapeHTML(tileMedia.position || "center")};"`
        : "";
      const tileClasses = [
        "category-tab",
        "category-tile",
        tileMedia ? "category-tile--with-image" : "",
        category.id === state.activeCategory && !state.dashboardOpen ? "active" : ""
      ].filter(Boolean).join(" ");
      const countLabel = `${category.count} ${plural(category.count, "блюдо", "блюда", "блюд")}`;

      return `
      <button class="${tileClasses}" type="button" data-category="${escapeHTML(category.id)}" ${category.disabled ? 'disabled aria-disabled="true"' : ""}${tileStyle}>
        <span class="category-tile__content">
          <strong>${escapeHTML(category.title)}</strong>
          <span class="category-tile__meta">${escapeHTML(countLabel)}</span>
        </span>
      </button>
    `;
    }).join("");
  ...
  function recipeCardContentTemplate(recipe, variantContext = null) {
    ...
    const showCategoryTextChip = state.activeCategory === "all" && !state.dashboardOpen;
    const categoryMarker = showCategoryTextChip
      ? `<span class="recipe-category-text-chip" aria-label="Категория: ${escapeHTML(categoryTitle)}">${escapeHTML(categoryTitle)}</span>`
      : `<span class="recipe-category-icon-badge recipe-category-icon-badge--${escapeHTML(recipe.category)}" aria-label="Категория: ${escapeHTML(categoryTitle)}">${categoryIconTemplate(recipe.category)}</span>`;
