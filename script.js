...
(() => {
  ...
  function updateCategoryEdgeMasks() {
    ...
    const scrollLeft = Math.min(Math.max(tabs.scrollLeft, 0), maxScroll);

    const leftProgress = maxScroll > 0 ? Math.min(scrollLeft / 46, 1) : 0;
    const rightProgress = maxScroll > 0 ? Math.min((maxScroll - scrollLeft) / 28, 1) : 0;

    tabs.style.setProperty("--filter-left-fade-soft", `${(10 * leftProgress).toFixed(2)}px`);
    tabs.style.setProperty("--filter-left-fade-mid", `${(24 * leftProgress).toFixed(2)}px`);
    tabs.style.setProperty("--filter-left-fade-full", `${(42 * leftProgress).toFixed(2)}px`);
    tabs.style.setProperty("--filter-right-fade", `${(22 * rightProgress).toFixed(2)}px`);

    tabs.classList.toggle("has-left-fade", leftProgress > 0);
    tabs.classList.toggle("no-right-fade", rightProgress <= 0);
  }
  ...
  function syncRecipeCardState(card, recipeId) {
    ...
    if (details) details.classList.toggle("visible", expanded);
