/* Portionly — static app logic, dashboard menu + split recipes + profile history + cart planning v110 */
(() => {
  "use strict";
  ...
  const state = {
    ...
    profileRecipeId: null,
    pendingCookedRecipeId: null,
    pendingRating: 4.5,
    aboutOpen: false,
    expandedIds: new Set(),
  ...
  function bindStaticEvents() {
    ...
    r.aboutToggleBtn?.addEventListener("click", openProfile);
    ...
    document.addEventListener("keydown", event => {
      if (event.key === "Escape") {
        if (state.pendingCookedRecipeId) closeCookedRatingModal();
        else if (state.profileOpen && state.profileMode !== "overview") navigateProfileBack();
  ...
  function renderTopbarState() {
    ...
    const listMode = state.profileOpen || !state.dashboardOpen || hasQuery;
  ...
  function showDashboard() {
    ...
    state.profileMode = "overview";
    state.profileCategory = null;
    state.profileRecipeId = null;
  ...
  function renderSearchState() {
    ...
    shell.classList.toggle("profile-open", state.profileOpen);
    if (state.profileOpen) {
      shell.classList.add("search-open");
      shell.classList.remove("dashboard-open", "list-open", "has-query");
      panel.hidden = true;
      panel.setAttribute("aria-hidden", "true");
      if (state.refs.categoryTabs) state.refs.categoryTabs.hidden = true;
  ...
  function handleCardsGridClick(event) {
    ...
    if (action) {
      ...
      if (actionName === "add-cart") addRecipeToCart(recipeId);
      if (actionName === "start-cooking") focusRecipeMethod(recipeId);
      if (actionName === "cooked-done") openCookedRatingModal(recipeId);
      if (actionName === "profile-category") openProfileCategory(action.dataset.categoryId);
      if (actionName === "profile-recipe") openProfileRecipe(action.dataset.recipeId);
      if (actionName === "profile-open-catalog") openCatalogRecipeFromProfile(action.dataset.recipeId);
      if (actionName === "profile-back") navigateProfileBack();
  ...
  function handleRecipeCardPointerDown(event) {
    const card = event.target.closest("[data-card-id]");
    const pages = event.target.closest(".recipe-card-pages");
  ...
  function handleRecipeCardPointerEnd() {
    const gesture = state.cardGesture;
    if (gesture?.card && gesture.moved) {
      window.setTimeout(() => {
        gesture.card.dataset.swipeIntent = "false";
      }, 320);
    }
    state.cardGesture = null;
  }

  function handleRecipeCardPagesScroll(event) {
    const pages = event.target;
    if (!pages?.classList?.contains("recipe-card-pages")) return;
    syncRecipeCardPager(pages);
  }
  ...
  function syncAllRecipeCardSizes() {
    state.refs.cardsGrid?.querySelectorAll(".recipe-card-pages").forEach(pages => {
      syncRecipeCardSize(pages);
      syncRecipeCardPager(pages);
    });
  }

  function getRecipeCardPageIndex(pages) {
    const width = Math.max(1, pages.clientWidth);
    return clamp(Math.round(pages.scrollLeft / width), 0, 2);
  }

  function syncRecipeCardPager(pages) {
    if (!pages?.classList?.contains("recipe-card-pages")) return;

    const card = pages.closest("[data-card-id]");
    if (!card || card.hidden || card.classList.contains("is-hidden")) return;

    const page = getRecipeCardPageIndex(pages);
    card.querySelectorAll("[data-recipe-dot]").forEach((dot, index) => {
      dot.classList.toggle("active", index === page);
    });
  }

  function syncRecipeCardSize(pages) {
    if (!pages?.classList?.contains("recipe-card-pages")) return;

    const card = pages.closest("[data-card-id]");
    if (!card || card.hidden || card.classList.contains("is-hidden")) return;

    const coverPage = pages.querySelector(".recipe-card-page--cover");
    if (!coverPage) return;

    const currentLeft = pages.scrollLeft;
    pages.style.height = "";
    pages.style.removeProperty("--recipe-card-fixed-height");

    const fixedHeight = Math.ceil(coverPage.scrollHeight);
    if (fixedHeight > 0) {
      pages.style.setProperty("--recipe-card-fixed-height", `${fixedHeight}px`);
      pages.style.height = `${fixedHeight}px`;
      pages.scrollLeft = currentLeft;
    }
  }
  ...
  function renderAll() {
    try {
      ...
      console.error("Portionly render error:", error);
      recoverDashboardAfterRenderError(error);
    }
  }

  function recoverDashboardAfterRenderError(error) {
    const tabs = state.refs.categoryTabs;
    ...
    if (tabs && !tabs.innerHTML.trim()) {
      ...
      const items = categories.length ? categories : [{ id: "all", title: "Все блюда" }];

      tabs.hidden = false;
      tabs.innerHTML = items.map(category => {
        const count = category.id === "all"
          ? recipes.length
          : recipes.filter(recipe => recipe.category === category.id).length;

        return `
          <button class="category-tab category-tile" type="button" data-category="${escapeHTML(category.id)}">
            <span class="category-tile__content">
              <strong>${escapeHTML(category.title)}</strong>
              <small>${count} ${plural(count, "блюдо", "блюда", "блюд")}</small>
            </span>
          </button>`;
      }).join("");
    ...
    if (grid) {
      ...
      if (!grid.innerHTML.trim()) {
        grid.innerHTML = `<div class="empty-state">Экран восстановлен после ошибки рендера. Обнови страницу, если данные не появились.</div>`;
      }
    }
  }
  ...
  function renderCategories() {
    ...
    state.refs.categoryTabs.innerHTML = categoryItems.map(category => `
      <button class="category-tab category-tile ${category.id === state.activeCategory && !state.dashboardOpen ? "active" : ""}" type="button" data-category="${escapeHTML(category.id)}" ${category.disabled ? 'disabled aria-disabled="true"' : ""}>
        <span class="category-tile__top">
          <span class="category-tab__icon category-tile__icon" aria-hidden="true">${categoryIconTemplate(category.id)}</span>
          <span class="category-tile__count" aria-label="${category.disabled ? "Пока пусто" : `${category.count} ${plural(category.count, "блюдо", "блюда", "блюд")}`}">${category.count}</span>
        </span>
        <span class="category-tile__content">
          <strong>${escapeHTML(category.title)}</strong>
        </span>
  ...
  function renderRecipes(options = {}) {
    ...
    if (state.refs.cardsGrid) state.refs.cardsGrid.classList.remove("profile-view");

    if (!showRecipeList) {
      if (state.refs.cardsGrid) state.refs.cardsGrid.hidden = true;
  ...
  function syncRecipeCardState(card, recipeId) {
    ...
    syncRecipeCardPager(card.querySelector(".recipe-card-pages"));
  }
  ...
  function getVisibleRecipes() {
    ...
    return sortRecipesByCookTime(list);
  }
    ...
    return `
      <div class="recipe-details" id="recipe-details-${escapeHTML(recipe.id)}">
        ...
        <div class="recipe-action-row">
          <button class="recipe-secondary-action" type="button" data-action="add-cart" data-recipe-id="${escapeHTML(recipe.id)}">
            <span>${state.selectedIds.includes(recipe.id) ? "В корзине" : "Добавить в корзину"}</span>
          </button>
          <button class="recipe-primary-action" type="button" data-action="start-cooking" data-recipe-id="${escapeHTML(recipe.id)}">
            <span>Начать готовить</span>
          </button>
        </div>

        <section class="detail-card detail-card--steps" data-method-for="${escapeHTML(recipe.id)}">
          <div class="detail-head">
        ...
        <section class="recipe-serve-shell">
          <button class="recipe-serve-toggle" type="button" data-action="cooked-done" data-recipe-id="${escapeHTML(recipe.id)}">
            <span>Подано к столу</span>
            <small>Сохранить в профиль и оценить блюдо</small>
            <b aria-hidden="true">›</b>
          </button>
        </section>
      </div>`;
  ...
  function renderProfileView() {
    ...
    grid.hidden = false;
    grid.className = "cards-grid profile-view";
    state.cardsRendered = false;

    const cookedRecords = getCookedRecords();
    const profileTitle = state.refs.recipeListTitle;
    const profileSubtitle = state.refs.recipeListSubtitle;
    if (state.refs.recipeListHead) state.refs.recipeListHead.hidden = false;

    if (state.profileMode === "recipe" && state.profileRecipeId) {
      const recipe = getRecipe(state.profileRecipeId);
      if (profileTitle) profileTitle.textContent = recipe?.title || "Статистика";
      if (profileSubtitle) profileSubtitle.textContent = "История приготовления блюда";
      grid.innerHTML = recipe ? profileRecipeTemplate(recipe) : `<div class="profile-empty">Блюдо не найдено.</div>`;
      return;
    }

    if (state.profileMode === "category" && state.profileCategory) {
      const category = getCategories().find(item => item.id === state.profileCategory);
      const summaries = getCookedRecipeSummaries(state.profileCategory);
      if (profileTitle) profileTitle.textContent = category?.title || "Раздел";
      if (profileSubtitle) profileSubtitle.textContent = `${summaries.length} ${plural(summaries.length, "блюдо", "блюда", "блюд")} в истории`;
      grid.innerHTML = profileCategoryTemplate(category, summaries);
      return;
    }

    if (profileTitle) profileTitle.textContent = "Профиль";
    if (profileSubtitle) profileSubtitle.textContent = cookedRecords.length
      ? `${cookedRecords.length} ${plural(cookedRecords.length, "приготовление", "приготовления", "приготовлений")} сохранено`
      : "История приготовлений пока пустая";
    grid.innerHTML = profileOverviewTemplate();
  }

  function profileOverviewTemplate() {
    const records = getCookedRecords();
    const total = records.length;
    const rated = records.map(item => Number(item.rating)).filter(Number.isFinite);
    const avg = rated.length ? rated.reduce((sum, value) => sum + value, 0) / rated.length : null;
    const last = records[0] || null;
    const categoryRows = getCategories()
      .filter(category => category.id !== "all")
      .map(category => {
        const count = records.filter(record => getRecipe(record.recipeId)?.category === category.id).length;
        const media = getProfileCategoryMedia(category.id);
        const style = media ? ` style="--tile-image: url('${escapeHTML(media.image)}'); --tile-image-position: ${escapeHTML(media.position || "center")};"` : "";
        return `
          <button class="profile-category-card ${count ? "" : "is-empty"}" type="button" data-action="profile-category" data-category-id="${escapeHTML(category.id)}"${style}>
            <span><strong>${escapeHTML(category.title)}</strong><small>${count} ${plural(count, "приготовление", "приготовления", "приготовлений")}</small></span>
          </button>`;
      }).join("");
    const recommendedRows = getRecommendedRecipes(getRecipes(), 3)
      .map(profileRecommendationCardTemplate)
      .join("");
    ...
    return `
      <section class="profile-panel">
        <div class="profile-hero">
          ...
          <strong>${total ? `${total} ${plural(total, "приготовление", "приготовления", "приготовлений")}` : "Пока пусто"}</strong>
          <p>${last ? `Последнее: ${escapeHTML(getRecipe(last.recipeId)?.title || "Блюдо")} · ${formatCookedDate(last.cookedAt)}` : "Готовые блюда будут появляться здесь после кнопки «Подано к столу»."}</p>
        </div>
        <div class="profile-stat-row">
          <div><strong>${total}</strong><span>всего</span></div>
          <div><strong>${avg === null ? "—" : formatRating(avg)}</strong><span>средняя оценка</span></div>
          <div><strong>${new Set(records.map(item => item.recipeId)).size}</strong><span>блюд</span></div>
        </div>
        <h3 class="profile-section-title">Приготовлено по разделам</h3>
        <div class="profile-category-grid">${categoryRows}</div>
        <h3 class="profile-section-title">Рекомендации дня</h3>
        <div class="profile-cooked-grid">${recommendedRows}</div>
      </section>`;
  }


  function profileRecommendationCardTemplate(recipe) {
    const image = getRecipeImage(recipe);
    const stats = getCookedRecipeStats(recipe.id);
    const rating = stats.avgRating === null ? "новое" : `${formatRating(stats.avgRating)} из 5`;
    return `
      <button class="profile-cooked-card profile-cooked-card--recommend" type="button" data-action="profile-open-catalog" data-recipe-id="${escapeHTML(recipe.id)}">
        <span class="profile-cooked-card__image">
        ...
        <span class="profile-cooked-card__copy">
          <strong>${escapeHTML(recipe.title)}</strong>
          <small>${escapeHTML(getCookTimeLabel(recipe))} · ${formatNumber(recipe.nutrition?.kcal || 0)} ккал</small>
          <em>${rating}</em>
        </span>
      </button>`;
  }

  function getProfileCategoryMedia(categoryId) {
    const map = {
      breakfast: { image: "assets/omelet-cheese-tomato-toast.jpg", position: "center 48%" },
      main: { image: "assets/chicken-bulgur-light-plov.jpg", position: "center 48%" },
      dinner: { image: "assets/fish-potato-cauliflower-puree.jpg", position: "center 48%" },
      dessert: { image: "assets/cottage-cheese-berry-dessert.jpg", position: "center 48%" }
    };
    return map[String(categoryId || "")] || null;
  }

  function profileCategoryTemplate(category, summaries) {
    const cards = summaries.map(({ recipe, stats }) => profileCookedCardTemplate(recipe, stats)).join("");
    return `
      <section class="profile-panel">
        <button class="profile-inline-back" type="button" data-action="profile-back">← Все разделы</button>
        ${summaries.length ? `<div class="profile-cooked-grid">${cards}</div>` : `<div class="profile-empty">В этом разделе пока нет приготовленных блюд.</div>`}
      </section>`;
  }

  function profileCookedCardTemplate(recipe, stats) {
    const image = getRecipeImage(recipe);
    return `
      <button class="profile-cooked-card" type="button" data-action="profile-recipe" data-recipe-id="${escapeHTML(recipe.id)}">
        <span class="profile-cooked-card__image">
          ${image ? `<img src="${escapeHTML(image)}" alt="${escapeHTML(recipe.title)}" loading="lazy" decoding="async" onerror="window.handleRecipeImageError(this)">` : ""}
        </span>
        <span class="profile-cooked-card__copy">
          <strong>${escapeHTML(recipe.title)}</strong>
          <small>${stats.count} ${plural(stats.count, "раз", "раза", "раз")} · ${stats.lastDate ? formatCookedDate(stats.lastDate) : "без даты"}</small>
          <em>${stats.avgRating === null ? "Без оценки" : `${formatRating(stats.avgRating)} из 5`}</em>
        </span>
      </button>`;
  }

  function profileRecipeTemplate(recipe) {
    const stats = getCookedRecipeStats(recipe.id);
    const image = getRecipeImage(recipe);
    const events = stats.records.slice(0, 8).map(record => `
      <div class="profile-history-row">
        <span>${formatCookedDate(record.cookedAt)}</span>
        <strong>${record.rating === null ? "без оценки" : `${formatRating(record.rating)} / 5`}</strong>
      </div>`).join("");

    return `
      <section class="profile-panel profile-recipe-detail">
        <button class="profile-inline-back" type="button" data-action="profile-back">← К списку</button>
        <div class="profile-detail-card">
          <div class="profile-detail-card__image">${image ? `<img src="${escapeHTML(image)}" alt="${escapeHTML(recipe.title)}" loading="lazy" decoding="async" onerror="window.handleRecipeImageError(this)">` : ""}</div>
          <div class="profile-detail-card__copy">
            <h3>${escapeHTML(recipe.title)}</h3>
            <div class="profile-stat-row profile-stat-row--compact">
              <div><strong>${stats.count}</strong><span>готовил</span></div>
              <div><strong>${stats.avgRating === null ? "—" : formatRating(stats.avgRating)}</strong><span>оценка</span></div>
              <div><strong>${stats.lastDate ? formatCookedDate(stats.lastDate, true) : "—"}</strong><span>последний раз</span></div>
            </div>
          </div>
        </div>
        <h3 class="profile-section-title">История</h3>
        <div class="profile-history-list">${events || `<div class="profile-empty">Записей пока нет.</div>`}</div>
      </section>`;
  }

  function openProfileCategory(categoryId) {
    state.profileMode = "category";
    state.profileCategory = categoryId;
    state.profileRecipeId = null;
    renderProfileView();
  }

  function openProfileRecipe(recipeId) {
    state.profileMode = "recipe";
    state.profileRecipeId = recipeId;
    renderProfileView();
  }

  function navigateProfileBack() {
    if (!state.profileOpen) return;
    if (state.profileMode === "recipe") {
      state.profileMode = state.profileCategory ? "category" : "overview";
      state.profileRecipeId = null;
    } else if (state.profileMode === "category") {
      state.profileMode = "overview";
      state.profileCategory = null;
    } else {
      showDashboard();
  ...
  function openCatalogRecipeFromProfile(recipeId) {
    const recipe = getRecipe(recipeId);
    if (!recipe) return;
    state.profileOpen = false;
    state.profileMode = "overview";
    ...
    state.profileRecipeId = null;
    state.dashboardOpen = false;
    state.activeCategory = recipe.category || "all";
    state.selectedOnly = false;
    state.search = "";
    state.searchOpen = false;
    if (state.refs.searchInput) state.refs.searchInput.value = "";
    ...
    renderRecipes();
    requestAnimationFrame(() => {
      getRecipeCardElement(recipeId)?.scrollIntoView?.({ behavior: "smooth", block: "center" });
    });
  }
  ...
  function addRecipeToCart(recipeId) {
    ...
    saveJson(STORAGE_SELECTED, state.selectedIds);
    updateRecipeSelectionUI(recipeId);
    const pageRoot = getRecipePageRoot(recipeId);
    const addBtn = pageRoot?.querySelector(`[data-action="add-cart"][data-recipe-id="${cssEscape(recipeId)}"] span`);
    if (addBtn) addBtn.textContent = "В корзине";
    updateSelectionUI();
    if (isModalOpen()) renderBasketModal();
    showToast("Добавлено в корзину", "success");
  }

  function focusRecipeMethod(recipeId) {
    const card = getRecipeCardElement(recipeId);
    const method = getRecipePageRoot(recipeId, card)?.querySelector(`[data-method-for="${cssEscape(recipeId)}"]`) || card?.querySelector(".detail-card--steps");
    if (method) {
      method.scrollIntoView({ behavior: "smooth", block: "center" });
      showToast("Способ приготовления открыт", "neutral");
    }
  }

  function openCookedRatingModal(recipeId) {
    if (!getRecipe(recipeId)) return;
    state.pendingCookedRecipeId = recipeId;
    state.pendingRating = 4.5;
    renderCookedRatingModal();
  }

  function closeCookedRatingModal() {
    state.pendingCookedRecipeId = null;
    const modal = document.getElementById("cookedRatingModal");
    if (modal) modal.remove();
  }

  function renderCookedRatingModal() {
    const recipe = getRecipe(state.pendingCookedRecipeId);
    if (!recipe) return;
    let modal = document.getElementById("cookedRatingModal");
    if (!modal) {
      modal = document.createElement("div");
      modal.id = "cookedRatingModal";
      modal.className = "cooked-rating-modal visible";
      document.body.appendChild(modal);
    }
    modal.innerHTML = `
      <section class="cooked-rating-panel" role="dialog" aria-modal="true" aria-label="Оценить блюдо">
        <button class="cooked-rating-close" type="button" data-rating-close aria-label="Закрыть">×</button>
        <span class="eyebrow">Подано к столу</span>
        <h2>${escapeHTML(recipe.title)}</h2>
        <p>Оцени блюдо — это попадёт в профиль и поможет рекомендациям.</p>
        <div class="cooked-rating-value"><strong data-rating-value>${formatRating(state.pendingRating)}</strong><span>из 5</span></div>
        <input class="cooked-rating-range" type="range" min="0.5" max="5" step="0.5" value="${state.pendingRating}" data-cooked-rating>
        <div class="cooked-rating-actions">
          <button class="tiny-btn ghost" type="button" data-rating-skip>Без оценки</button>
          <button class="tiny-btn" type="button" data-rating-save>Сохранить</button>
        </div>
      </section>`;
  }
  ...
  function handleDocumentAction(event) {
    if (event.target.closest("[data-rating-close]")) {
      event.preventDefault();
      closeCookedRatingModal();
      return;
    }
    if (event.target.id === "cookedRatingModal") {
      closeCookedRatingModal();
      return;
    }
    if (event.target.closest("[data-rating-skip]")) {
      event.preventDefault();
      saveCookedRecipe(state.pendingCookedRecipeId, null);
      closeCookedRatingModal();
      return;
    }
    if (event.target.closest("[data-rating-save]")) {
      event.preventDefault();
      saveCookedRecipe(state.pendingCookedRecipeId, state.pendingRating);
      closeCookedRatingModal();
      return;
    }
  }
  ...
  function handleDocumentInput(event) {
    const input = event.target.closest?.("[data-cooked-rating]");
    if (!input) return;
    state.pendingRating = Number(input.value) || 4.5;
    const label = document.querySelector("[data-rating-value]");
    if (label) label.textContent = formatRating(state.pendingRating);
  }

  function saveCookedRecipe(recipeId, rating) {
    const recipe = getRecipe(recipeId);
    if (!recipe) return;
    const plan = getRecipePlan(recipeId);
    ...
    const record = {
      ...
      rating: Number.isFinite(numericRating) ? clamp(Math.round(numericRating * 2) / 2, 0.5, 5) : null
    };
    state.cookedHistory = normalizeCookedHistory([record, ...state.cookedHistory]).slice(0, 500);
    saveJson(STORAGE_COOKED_HISTORY, state.cookedHistory);
    showToast("Сохранено в профиль", "success");
    if (state.profileOpen) renderProfileView();
  }

  function getCookedRecords() {
    return normalizeCookedHistory(state.cookedHistory)
      .filter(record => getRecipe(record.recipeId))
      .sort((a, b) => new Date(b.cookedAt).getTime() - new Date(a.cookedAt).getTime());
  }

  function getCookedRecipeStats(recipeId) {
    const records = getCookedRecords().filter(record => record.recipeId === recipeId);
    const ratings = records.map(record => Number(record.rating)).filter(Number.isFinite);
    return {
      records,
      count: records.length,
      lastDate: records[0]?.cookedAt || null,
      avgRating: ratings.length ? ratings.reduce((sum, value) => sum + value, 0) / ratings.length : null
    };
  }

  function getCookedRecipeSummaries(categoryId = null) {
    const grouped = new Map();
    getCookedRecords().forEach(record => {
      const recipe = getRecipe(record.recipeId);
      if (!recipe) return;
      if (categoryId && recipe.category !== categoryId) return;
      if (!grouped.has(recipe.id)) grouped.set(recipe.id, recipe);
    });
    return [...grouped.values()].map(recipe => ({ recipe, stats: getCookedRecipeStats(recipe.id) }));
  }
  ...
  function normalizeCookedHistory(value) {
    ...
    return value.map(item => {
      ...
      if (!allowed.has(recipeId)) return null;
      const rating = Number(item?.rating);
      return {
        id: String(item?.id || `${Date.now()}-${recipeId}`),
        recipeId,
        cookedAt: item?.cookedAt || new Date().toISOString(),
        people: clamp(Number(item?.people) || 1, MIN_SETTING, MAX_PEOPLE),
        days: clamp(Number(item?.days) || 1, MIN_SETTING, MAX_DAYS),
        portions: Math.max(1, Number(item?.portions) || 1),
        rating: Number.isFinite(rating) ? clamp(Math.round(rating * 2) / 2, 0.5, 5) : null
      };
    }).filter(Boolean);
  }

  function getRecommendedRecipes(sourceRecipes = getRecipes(), limit = 12) {
    const now = Date.now();
    return [...sourceRecipes]
      .map((recipe, index) => {
        const stats = getCookedRecipeStats(recipe.id);
        const lastTime = stats.lastDate ? new Date(stats.lastDate).getTime() : 0;
        const daysAgo = lastTime ? (now - lastTime) / 86400000 : 999;
        const ratingBonus = stats.avgRating === null ? 0.7 : (stats.avgRating - 3) * 1.35;
        const novelty = stats.count ? 0 : 2.4;
        const recentPenalty = daysAgo < 3 ? 4 : daysAgo < 7 ? 1.8 : 0;
        const quickBonus = getCookTimeSortRange(recipe).max <= 25 ? 0.65 : 0;
        return { recipe, score: ratingBonus + novelty + quickBonus - recentPenalty - index * 0.002 };
      })
  ...
  function formatCookedDate(value, short = false) {
  ...
  function renderFatalError(message) {
    ...
    if (!grid) return;
    grid.hidden = false;
    grid.classList.remove("profile-view");
