/* Portionly — static app logic, swipe cards + cart planning v74 */
(() => {
  "use strict";
  ...
  function scheduleRecipeCardHeightSync() {
    ...
    state.recipeCardHeightTimer = window.setTimeout(syncAllRecipeCardPagers, 60);
  }

  function syncAllRecipeCardPagers() {
    state.refs.cardsGrid?.querySelectorAll(".recipe-card-pages").forEach(syncRecipeCardPager);
  }
  ...
  function syncRecipeCardPager(pages) {
    ...
    const activePage = pages.querySelectorAll(".recipe-card-page")[page];
    if (!activePage) return;

    const nextHeight = Math.ceil(activePage.scrollHeight);
    if (nextHeight > 0) pages.style.height = `${nextHeight}px`;
  }
