/* Portionly — static app logic, cart unified layout v20 */
(() => {
  "use strict";
  ...
  function renderBasketModal() {
    ...
    // v18: статистика больше не отдельная карточная сетка. Она встроена в блок списка продуктов,
    // чтобы корзина не выглядела как матрёшка из карточек.
    r.basketSummary.innerHTML = "";
    r.totalsList.innerHTML = basketUnifiedProductsTemplate(totals, selectedRecipes, totalPortions);
  }
  ...
  function basketUnifiedProductsTemplate(totals, selectedRecipes, totalPortions) {
    ...
    const summary = `
      <section class="basket-inline-summary" aria-label="Итоги корзины">
        <div>
          <small>Выбрано</small>
          <strong>${selectedRecipes.length} ${plural(selectedRecipes.length, "блюдо", "блюда", "блюд")}</strong>
        </div>
        <div>
          <small>Порции</small>
        ...
        <div>
          <small>Калории</small>
          <strong>${formatNumber(nutrition.kcal)} ккал</strong>
        </div>
        <div>
          <small>Б / Ж / У</small>
          <strong>${formatNumber(nutrition.protein)} / ${formatNumber(nutrition.fat)} / ${formatNumber(nutrition.carbs)}</strong>
        </div>
      </section>
    `;
  ...
  function ingredientGroupsTemplate(totals) {
    return basketUnifiedProductsTemplate(totals, [], 0);
  }
  ...
  function pantryIngredientTemplate(item) {
    ...
    return `
      <div class="total-item pantry-row ${checked ? "has-home" : ""}" data-pantry-key="${escapeHTML(key)}">
        <button class="pantry-check" type="button" data-pantry-toggle="${escapeHTML(key)}" data-needed="${escapeHTML(needed)}" data-unit="${escapeHTML(item.unit)}" aria-pressed="${checked}" aria-label="${checked ? "Убрать отметку, что ингредиент есть дома" : "Отметить, что ингредиент есть дома"}">
          ${checked ? "✓" : ""}
        </button>
  ...
  function handleTotalsListClick(event) {
    ...
    if (state.pantryMap[key]?.checked) {
      ...
      state.pantryMap[key] = { checked: true, amount: roundSmart(needed), unit };
    }
  ...
  function copyBasketText() {
    ...
    buildIngredientGroups(totals).filter(group => group.items.length).forEach(group => {
      ...
      group.items.forEach(item => {
        if (item.amount === null) {
          lines.push(`- ${item.name}: ${item.unit}`);
          return;
        }
        const pantry = getPantryState(item);
        const amountText = formatAmount(item.amount, item.unit);
        lines.push(pantry
          ? `- ${item.name}: ${formatNumber(pantry.amount)} из ${amountText} есть дома`
