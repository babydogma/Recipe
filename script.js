/* Portionly — static app logic, polished UX v14 */
(() => {
  "use strict";
  ...
  function renderBasketModal() {
    ...
    r.selectedStrip.innerHTML = selectedRecipes.map(recipe => {
      ...
      return `
        <button class="selected-chip" type="button" data-remove-id="${escapeHTML(recipe.id)}">
          <span>${escapeHTML(recipe.title)}</span><small>${portions} ${plural(portions, "порция", "порции", "порций")}</small><b>×</b>
        </button>
      `;
    }).join("");
    ...
    r.basketSummary.innerHTML = `
      <div class="summary-pill summary-pill--accent"><span>Выбрано</span><strong>${selectedRecipes.length} ${plural(selectedRecipes.length, "блюдо", "блюда", "блюд")}</strong></div>
    ...
    selectedRecipes.forEach(recipe => {
      ...
      getScaledIngredients(recipe, targetPortions).forEach(ingredient => {
        ...
        if (ingredient.amount === null || ingredient.amount === undefined) {
          tasteMap.set(normalizeName(name), { name, unit, group });
          return;
        }
  ...
  function ingredientGroupsTemplate(totals) {
    ...
    totals.taste.forEach(item => {
      const group = groups.get("taste");
      group.items.push({ ...item, amount: null, sources: [] });
    });

    const rendered = Array.from(groups.values())
      .filter(group => group.items.length)
      .map(group => `
        <section class="ingredient-group">
          <div class="ingredient-group__head">
          ...
          <div class="ingredient-group__items">
            ${group.items.map(item => `
              <div class="total-item">
                <div><strong>${escapeHTML(item.name)}</strong><small>${item.amount === null ? "по вкусу" : `${item.sources.length} ${plural(item.sources.length, "блюдо", "блюда", "блюд")}`}</small></div>
  ...
  const INGREDIENT_GROUPS = [
    ...
    { id: "sauces_spices", title: "Соусы, масло и специи", icon: "🧂" },
  ...
  function getIngredientGroup(name) {
    ...
    if (includesAny(value, ["курин", "фарш", "говядин", "рыб", "филе рыбы"])) return "meat_fish";
    if (includesAny(value, ["яй", "молоко", "сыр", "творог", "йогурт", "сливочное масло"])) return "dairy_eggs";
    ...
    if (includesAny(value, ["масло", "специ", "соль", "перец", "паприка", "чеснок", "лимон", "заправ", "соус"])) return "sauces_spices";
    if (includesAny(value, ["огур", "помид", "томат", "зелень", "салат", "карто", "капуст", "морков", "лук", "кабач", "перец", "овощ"])) return "vegetables";
  ...
  function copyBasketText() {
    ...
    INGREDIENT_GROUPS.forEach(group => grouped.set(group.id, { ...group, items: [] }));
    totals.numeric.forEach(item => grouped.get(grouped.has(item.group) ? item.group : "other").items.push(item));
    totals.taste.forEach(item => grouped.get("taste").items.push({ ...item, amount: null }));

    Array.from(grouped.values()).filter(group => group.items.length).forEach(group => {
