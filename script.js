...
const STORAGE_UI = "portionly:ui:v5";
const STORAGE_PANTRY = "portionly:pantry:v1";
...
let pantryMap = normalizePantryMap(loadJson(STORAGE_PANTRY, {}));
...
function resolveAssetPath(path) {
  ...
  const base = location.hostname.endsWith("github.io") ? "/Recipe/" : "";
  return `${base}${path}`;
}
...
function savePantry() {
  localStorage.setItem(STORAGE_PANTRY, JSON.stringify(pantryMap));
}
...
function normalizePantryMap(next) {
  ...
  return Object.entries(next).reduce((acc, [key, value]) => {
    ...
    if (!key || !Number.isFinite(amount)) return acc;
    acc[key] = {
      amount: Math.max(0, roundSmart(amount)),
      autoFull: Boolean(value?.autoFull)
    };
    return acc;
  }, {});
}
...
function renderBasketModal() {
  ...
  totalsList.innerHTML = totals.numeric.map(item => pantryIngredientTemplate(item)).join("");
...
function pantryIngredientTemplate(item) {
  ...
  const ownedAmount = checked ? pantry.amount : null;
  const neededText = formatAmount(item.amount, item.unit);

  return `
    <div class="total-item pantry-row ${checked ? "has-home" : ""}" data-pantry-key="${escapeHTML(item.key)}">
      <button class="pantry-check" type="button" data-pantry-toggle="${escapeHTML(item.key)}" aria-pressed="${checked}" aria-label="${checked ? "Убрать отметку, что ингредиент есть дома" : "Отметить, что ингредиент есть дома"}">
        ${checked ? "✓" : ""}
      </button>
      ...
      <div class="total-amount ${checked ? "owned-mode" : ""}">
        ${checked ? `
          <input class="owned-input" data-owned-input="${escapeHTML(item.key)}" inputmode="decimal" type="text" value="${formatNumber(ownedAmount)}" aria-label="Сколько есть дома">
          <span>из ${formatNumber(item.amount)} ${escapeHTML(item.unit)}</span>
        ` : `<span>${neededText}</span>`}
      </div>
    </div>`;
}

function bindPantryControls(totals) {
  const itemsByKey = new Map(totals.numeric.map(item => [item.key, item]));

  totalsList.querySelectorAll("[data-pantry-toggle]").forEach(button => {
    button.addEventListener("click", event => {
      event.stopPropagation();
      const key = button.dataset.pantryToggle;
      const item = itemsByKey.get(key);
      if (!item) return;

      if (pantryMap[key]) {
        delete pantryMap[key];
      } else {
        pantryMap[key] = { amount: roundSmart(item.amount), autoFull: true };
      }
  ...
  totalsList.querySelectorAll("[data-owned-input]").forEach(input => {
    ...
    input.addEventListener("input", () => {
      ...
      pantryMap[key] = { amount: roundSmart(amount), autoFull: false };
      savePantry();
    });
    ...
    input.addEventListener("blur", () => {
      ...
      if (!Number.isFinite(amount)) {
        pantryMap[key] = { amount: 0, autoFull: false };
      } else {
        pantryMap[key] = { amount: Math.max(0, roundSmart(amount)), autoFull: false };
      }
      savePantry();
      renderBasketModal();
    });
...
function getPantryState(item) {
  const saved = pantryMap[item.key];
  if (!saved) return null;

  const amount = saved.autoFull
    ? roundSmart(item.amount)
    : Math.max(0, roundSmart(Number(saved.amount) || 0));

  return { amount, autoFull: Boolean(saved.autoFull) };
}
...
function ingredientKey(name, unit) {
  return `${normalizeName(name)}__${String(unit).toLowerCase().trim()}`;
}
...
function calculateTotals(selectedRecipes, targetPortions) {
  ...
  selectedRecipes.forEach(recipe => {
    ...
    getScaledIngredients(recipe, targetPortions).forEach(ingredient => {
      ...
      if (!numericMap.has(key)) numericMap.set(key, { key, name, unit, amount: 0, sources: [] });
...
function copyBasketText() {
  ...
  totals.numeric.forEach(item => {
    const pantry = getPantryState(item);
    if (pantry) {
      lines.push(`- ${item.name}: есть дома ${formatNumber(pantry.amount)} из ${formatAmount(item.amount, item.unit)}`);
    } else {
