/* Portionly — static app logic, Figma-style dashboard + recipe dropdown cards + cart planning v99 */
(() => {
  "use strict";
  ...
  function renderCategoryOverview() {
    ...
    const otherRecipes = filteredRecipes.filter(recipe => recipe.id !== featured?.id).slice(0, 4);
  ...
  function handleRecipeScreenClick(event) {
    ...
    if (actionName === "jump-method") {
      event.preventDefault();
      state.refs.recipeScreen?.querySelector("[data-recipe-method]")?.scrollIntoView({ behavior: "smooth", block: "start" });
      return;
    }
  ...
  function recipeScreenTemplate(recipe) {
    ...
    const category = getShortCategoryTitle(getCategories().find(item => item.id === recipe.category));
    ...
    return `
      <article class="recipe-screen__panel" role="dialog" aria-modal="true" aria-label="${escapeHTML(recipe.title)}">
        ...
        <div class="recipe-screen__body">
          ...
          <button class="recipe-screen-start ${selected ? "selected" : ""}" type="button" data-action="start-cooking" data-recipe-id="${escapeHTML(recipe.id)}">
            <span aria-hidden="true">${chefIconMarkup()}</span>
            <strong>${selected ? "Открыть корзину" : "Начать готовить"}</strong>
          </button>

          <section class="recipe-screen-method" data-recipe-method aria-label="Способ приготовления">
            <div class="recipe-screen-method__head">
              <h3>Способ приготовления</h3>
              <span>${steps.length} ${plural(steps.length, "шаг", "шага", "шагов")}</span>
            </div>
            <div class="recipe-screen-steps">
              ${recipeScreenStepsTemplate(steps)}
            </div>
          </section>
  ...
  function recipeScreenNutritionItem(label, value, unit = "") {
    return `
      <div>
        ...
        <span>${escapeHTML(label)}${unit ? ` ${escapeHTML(unit)}` : ""}</span>
      </div>`;
  }
  ...
  function recipeScreenStepsTemplate(steps) {
  ...
  function chefIconMarkup() {
    return `
      <svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
        <path d="M7.3 10.4a4.1 4.1 0 0 1-.4-8.1 4.9 4.9 0 0 1 9.2 0 4.1 4.1 0 0 1 .6 8.1"/>
        <path d="M7.6 9.9h8.8l-.7 9.4a1.9 1.9 0 0 1-1.9 1.7H10a1.9 1.9 0 0 1-1.9-1.7l-.5-9.4Z"/>
        <path d="M9.4 14h5.2"/>
      </svg>`;
  }
  ...
  function updateRecipeScreenSelectionUI(recipeId) {
    ...
    if (label) label.textContent = selected ? "Открыть корзину" : "Начать готовить";
  }
